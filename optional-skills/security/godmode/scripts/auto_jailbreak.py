...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SAGE_HOME", os.path.expanduser("~/.sage")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SAGE_HOME", Path.home() / ".sage")) / "skills" / "red-teaming" / "godmode"
...
HERMES_HOME = Path(os.getenv("SAGE_HOME", Path.home() / ".sage"))
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.sage/prefill.json."""
