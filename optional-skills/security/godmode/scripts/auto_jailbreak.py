...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("AGENTIK_HOME", os.path.expanduser("~/.agentik")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("AGENTIK_HOME", Path.home() / ".agentik")) / "skills" / "red-teaming" / "godmode"
...
HERMES_HOME = Path(os.getenv("AGENTIK_HOME", Path.home() / ".agentik"))
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.agentik/prefill.json."""
