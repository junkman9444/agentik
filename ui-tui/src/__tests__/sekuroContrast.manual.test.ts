...
describe('typed composer text uses the same bright magenta as border/logo (otter_space: "typing still can\'t see, use all bright solid colors, same as logo and border")', () => {
  it('dark mode: theme.color.text (what TextInput renders) equals the bright accent hex', () => {
    const t = fromSkin(DARK_COLORS, {}, '', '', '', '')

    expect(t.color.text).toBe('#FF4FB8')
    expect(t.color.text).toBe(t.color.border)
  })

  it('light mode: theme.color.text equals the brand magenta hex', () => {
    const merged = { ...DARK_COLORS, ...LIGHT_COLORS }
    const t = fromSkin(merged, {}, '', '', '', '')

    expect(t.color.text).toBe('#C6007E')
    expect(t.color.text).toBe(t.color.border)
  })
})
