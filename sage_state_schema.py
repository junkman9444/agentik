...
Plain mixin for ``sage_state.SessionDB`` (no ``__init__``/state of its own).
Must never import sage_state (cycle); shared constants live in sage_state_common.
"""
...
from sage_state_holders import _read_proc_argv
...
class SessionSchemaMixin:
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        with contextlib.suppress(Exception):  # missing/corrupt cache → recompute below
            # Late import: resolves a test-patched sage_constants.get_hermes_home.
            from sage_constants import get_hermes_home as _home
