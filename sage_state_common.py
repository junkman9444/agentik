"""Shared constants and helpers for the SessionDB family of modules.  Lives outside sage_state so
...
# 'new_session' (sage_cli/cli_session_mixin.py) without a reset child row.  A compression rotation must
...
# claims progress under SQLite transaction authority).  Mirrors `sage_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import sage_state.
...
# authority and is intentionally multi-process. Semantics mirror `sage_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not sage_state) because the search/schema mixins
# cannot import sage_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
