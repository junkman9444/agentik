...
from sage_cli.active_sessions import active_session_registry_snapshot
from sage_cli.browser_connect import ChromeDebugLaunch
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    from sage_cli.tools_config import _RECENTLY_SHIPPED_TOOLSETS
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sage_state_registry.acquire", lambda db_path=None: profile_db)
    monkeypatch.setattr(server, "_get_db", lambda: launch_db)
    monkeypatch.setattr(server, "_enable_gateway_prompts", lambda: None)
    monkeypatch.setattr(server, "_set_session_context", lambda target: [])
    monkeypatch.setattr(server, "_clear_session_context", lambda tokens: None)
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "sage_cli.model_switch.resolve_persist_behavior",
    ...
    monkeypatch.setattr(
        "sage_cli.model_cost_guard.expensive_model_warning",
        lambda *a, **k: None,
    )
    session = {"agent": None}
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "sage_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "sage_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from sage_state_common import _RECOVERABLE_END_REASONS
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sage_state_registry.acquire", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_config_get_approval_mode_fails_safe_to_manual_for_invalid_explicit_value(
    ...
    # tools.approval, which reads via sage_cli.config.load_config —
...
def test_config_set_fast_updates_live_agent_session_scoped(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.models.resolve_fast_mode_overrides",
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("sage_cli.main._has_any_provider_configured", lambda **_kw: False)
...
def test_setup_status_answers_from_the_bootstrap_record_once_it_exists(monkeypatch):
    ...
    fb.reset_for_tests()
    monkeypatch.setattr("sage_cli.main._has_any_provider_configured",
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("sage_cli.main._has_any_provider_configured", lambda **_kw: True)
    monkeypatch.setattr(
        "sage_cli.runtime_provider.resolve_runtime_provider",
...
def test_setup_readiness_scopes_to_requested_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("sage_cli.profiles.profile_exists", lambda name: name == "bot")
    ...
    monkeypatch.setattr("sage_cli.runtime_provider.resolve_runtime_provider", fake_resolve)
...
def test_setup_readiness_unknown_profile_never_answers_for_launch_profile(monkeypatch):
    ...
    monkeypatch.setattr("sage_cli.profiles.profile_exists", lambda name: False)
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "sage_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=_Agent())
    monkeypatch.setattr("sage_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("sage_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_recovers_failed_profile_resume_after_build_completes(
    ...
    monkeypatch.setattr("sage_cli.model_switch.switch_model", fake_switch_model)
    monkeypatch.setattr(
        "sage_cli.model_selection_guards.combined_selection_warning",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr("sage_state_registry.acquire", FakeDb)
    monkeypatch.setattr(server, "_make_agent", fake_make_agent)
...
def test_config_set_model_session_switch_clears_pending_once_restore(monkeypatch):
    ...
    server._sessions["sid"] = session
    monkeypatch.setattr("sage_cli.model_switch.switch_model", lambda **_kwargs: result)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda *args, **kwargs: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_personality_preserves_history_and_returns_info(monkeypatch):
    ...
    monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
    # Persistence now flows through the single owner (sage_cli.personality),
    # never _write_config_key / agent.system_prompt.
    import sage_cli.personality as personality_mod
...
def test_commands_catalog_includes_plugin_commands(monkeypatch):
    monkeypatch.setattr(
        "sage_cli.plugins.get_plugin_commands",
        lambda: {
            "lcm": {
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("sage_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "sage_cli.commands_completion.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("sage_state")
    ...
    monkeypatch.setitem(sys.modules, "sage_state", fake_mod)
    fake_registry = types.ModuleType("sage_state_registry")
    fake_registry.acquire = _broken_shared
    monkeypatch.setitem(sys.modules, "sage_state_registry", fake_registry)
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("sage_state_registry.acquire", ProfileDB)
...
def test_session_most_recent_honors_params_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr("sage_state_registry.acquire", ProfileDB2)
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    with patch(
        ...
        # guarding against.
        with patch("sage_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from sage_cli.inventory import ConfigContext
    ...
    monkeypatch.setattr(
        "sage_cli.inventory.build_models_payload",
...
def test_model_options_preserves_canonical_custom_row_after_agent_init(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.runtime_provider.canonical_custom_identity",
        canonical,
    )
    monkeypatch.setattr(
        "sage_cli.model_switch.list_authenticated_providers",
    ...
    monkeypatch.setattr(
        "sage_cli.auth.is_provider_explicitly_configured",
        lambda _slug: False,
    )
    monkeypatch.setattr("sage_cli.inventory._apply_pricing", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("sage_cli.inventory._apply_capabilities", lambda *_args, **_kwargs: None)
...
def test_model_save_key_uses_credential_lifecycle_and_picker_context(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.auth.PROVIDER_REGISTRY",
        {
    ...
    monkeypatch.setattr("sage_cli.config.is_managed", lambda: False)
    save_credential = Mock()
    monkeypatch.setattr(
        "sage_cli.credential_lifecycle.save_provider_env_credential",
        save_credential,
    )
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"sage_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        ...
        with (
            patch(
                "sage_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("sage_cli.browser_connect.local_port_in_use", return_value=False),
            patch("sage_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "sage_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_squatted_port_launches_on_alternate(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool_lifecycle": fake}):
        with (
            patch("sage_cli.browser_connect.launch_chrome_debug", side_effect=_launch),
            patch("sage_cli.browser_connect.local_port_in_use", return_value=True),
            patch("sage_cli.browser_connect.find_free_debug_port", return_value=9223),
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_hermes_cli_reload_env(monkeypatch):
    ...
    with patch.dict(sys.modules, {"sage_cli.config": fake}):
...
def test_reap_idle_sessions_calls_periodic_trim(monkeypatch):
    ...
    # `from sage_cli.mem_trim import trim_memory` at call time.
    import sage_cli.mem_trim as mem_trim
...
def test_billing_error_serialization_preserves_server_code(
    ...
    import sage_cli.nous_billing as nb
...
def test_prompt_submit_releases_old_history_before_heap_trim(monkeypatch, tmp_path):
    ...
    try:
        ...
        monkeypatch.setattr("sage_cli.mem_trim.trim_memory", _inspect_trim_frame)
...
def test_prompt_submit_truncation_archives_instead_of_deleting(monkeypatch):
    ...
    tests/sage_state/test_replace_messages_archive_siblings.py.
    """
