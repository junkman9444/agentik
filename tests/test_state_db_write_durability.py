...
The defect: sage_state already knows macOS `fsync()` does not guarantee
...
import sage_state
from sage_state import repair_state_db_schema
from sage_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in sage_state_repair; sage_state only
    # re-imports them.
    import sage_state_repair

    source = Path(sage_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(sage_state_repair.__file__))
