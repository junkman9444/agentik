...
def test_subprocess_pkill_python_dash_f_blocked():
    """``pkill -f python`` matches the gateway's "python -m sage_cli.main"."""
    with pytest.raises(RuntimeError, match="live-system guard"):
...
def test_subprocess_popen_real_gateway_restart_blocked():
    """``python -m sage_cli.main gateway restart`` is a detached child that
    ...
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.Popen(
            [sys.executable, "-m", "sage_cli.main", "gateway", "restart"],
...
def test_subprocess_run_gateway_status_passes_through():
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import sys; print(sys.argv[1:])", "-m", "sage_cli.main", "gateway", "status"],
