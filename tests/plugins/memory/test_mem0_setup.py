...
def _inject_fake_hermes_cli(monkeypatch):
    """Inject fake sage_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("sage_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("sage_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_hermes_cli = types.ModuleType("sage_cli")
    fake_hermes_cli.config = fake_config_mod
    fake_hermes_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "sage_cli", fake_hermes_cli)
    monkeypatch.setitem(sys.modules, "sage_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "sage_cli.memory_setup", fake_setup_mod)
