...
guard is covered in ``tests/sage_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``sage_cli.model_switch_providers.prewarm_picker_cache_async``
...
import sage_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from sage_cli.model_switch,
