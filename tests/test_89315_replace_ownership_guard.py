...
def profile_env(tmp_path, monkeypatch):
    """Isolated HERMES_HOME mirroring tests/sage_cli/test_profiles.py."""
...
def _record(pid=424242, start=111222333, home=None, argv=None):
    return {
        "pid": pid,
        "kind": "hermes-gateway",
        "argv": argv or ["python", "-m", "sage_cli.main", "gateway", "run"],
        "start_time": start,
        "hermes_home": home,
    }
...
class TestArgvConsistencyCheck:
    ...
    def test_prefix_collision_is_not_a_conflict(self, profile_env):
        ...
        assert (
            conflict("python -m sage_cli.main --profile timothy gateway run", tim_home)
            is True
        )
        # Our own exact name stays consistent.
        assert (
            conflict("python -m sage_cli.main --profile tim gateway run", tim_home)
            is False
        )
        assert (
            conflict("python -m sage_cli.main -p tim gateway run", tim_home)
            is False
        )
    ...
    def test_explicit_home_flag_exact_compare(self, profile_env):
        ...
        assert (
            conflict(
                "python -m sage_cli.main HERMES_HOME=/home/x/.hermes/profiles/timothy gateway run",
                tim_home,
            )
            is True
        )
        assert (
            conflict(
                "python -m sage_cli.main --hermes-home /home/x/.hermes/profiles/tim/ gateway run",
                tim_home,
            )
    ...
    def test_consistency_contradiction_refuses_even_with_agreeing_record(
        ...
        with (
            ...
            patch(
                ...
                return_value="python -m sage_cli.main --profile other-profile gateway run",
            ),
            patch(
                "gateway.status._get_process_hermes_home",
                return_value=profile_env / ".hermes",
