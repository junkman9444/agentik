...
class TestCreditsNoticesToggle:
    def test_disabled_emits_nothing(self):
        ...
        with patch("sage_cli.config.load_config", return_value=_cfg(False)):
    ...
    def test_config_error_fails_open(self):
        ...
        received = []
        agent.notice_callback = received.append
        with patch("sage_cli.config.load_config", side_effect=RuntimeError("boom")):
            agent._emit_credits_notices()
    ...
    def test_toggle_cached_per_agent(self):
        ...
        with patch("sage_cli.config.load_config", return_value=_cfg(True)) as mock_load:
            agent._emit_credits_notices()
            agent._emit_credits_notices()
