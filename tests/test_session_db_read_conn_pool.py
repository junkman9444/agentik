...
``sage_cli.sqlite_safe_read``'s registry permanently over-counted as well.
...
def test_read_open_failure_backs_off_but_recovers(db):
    ...
    from sage_state import _READ_OPEN_RETRY_SECONDS
...
def test_peak_live_connections_bounded_under_simultaneous_burst(db):
    ...
    """
    from sage_state import _READ_POOL_MAX
...
def test_peak_is_bounded_across_two_SessionDBs_on_one_path(db):
    ...
    from sage_state import SessionDB, _READ_POOL_MAX
...
def test_peak_is_bounded_across_many_database_files(tmp_path):
    ...
    import sage_state
    from sage_state import SessionDB, _READ_POOL_MAX
    from sage_state_readpool import _READ_POOL_PROCESS_MAX
    ...
    try:
        ...
        for d in dbs:
            d.close()
        assert sage_state_readpool._process_read_permits.acquire(blocking=False), (
            "close() stranded a process permit"
        )
        sage_state_readpool._process_read_permits.release()
...
def test_no_read_connection_is_opened_without_descriptor_headroom(db, monkeypatch):
    ...
    """
    import sage_state_readpool as readpool
...
def test_duplicate_handles_on_one_path_are_reported(db, caplog):
    ...
    from sage_state import SessionDB
    from sage_state_readpool import _HANDLES_PER_PATH_WARN
