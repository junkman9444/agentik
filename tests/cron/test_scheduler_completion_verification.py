...
def _run_booked_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sage_state, "SessionDB", _RecordingSessionDB)
    monkeypatch.setattr(run_agent, "AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "sage_constants.resolve_reasoning_config", lambda *_a, **_k: None
    )
    ...
    monkeypatch.setattr(
        "sage_cli.runtime_provider.resolve_runtime_provider", _fake_runtime
    )
