...
def test_named_profile_resolves_when_exists():
    with mock.patch("sage_cli.profiles.profile_exists", return_value=True):
...
def test_unknown_profile_resolves_to_none():
    with mock.patch("sage_cli.profiles.profile_exists", return_value=False):
...
def test_delivery_targets_include_local_profiles():
    with mock.patch("sage_cli.profiles.list_profile_names",
