"""Tests for sage_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports sage_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported sage_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("sage_bootstrap", None)
    import sage_bootstrap  # noqa: WPS433
    return sage_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import sage_bootstrap as its
    ...
    # import sage_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "sage_cli/main.py",   # hermes CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import sage_bootstrap' and that
        ...
        ``sage_bootstrap`` as a top-level module) leaves hermes
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when sage_bootstrap hasn't been
        ...
        assert first_import_name == "sage_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'sage_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import sage_bootstrap' line to be the first import."
        )
