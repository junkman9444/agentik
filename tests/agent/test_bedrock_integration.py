...
Note: Tests that import ``sage_cli.auth`` or ``sage_cli.runtime_provider``
...
class TestProviderAliases:
    ...
    def test_aws_alias(self):
        from sage_cli.models import _PROVIDER_ALIASES
...
class TestResolveProvider:
    ...
    def test_auto_detect_with_aws_credentials(self, monkeypatch):
        ...
        monkeypatch.setattr("sage_cli.anon_auth.guest_enabled", lambda: False)
        # Mock the auth store to have no active provider
        with patch("sage_cli.auth._load_auth_store", return_value={}):
...
class TestRuntimeProvider:
    ...
    def test_bedrock_runtime_no_credentials_raises_on_auto_detect(self, monkeypatch):
        ...
        with patch("sage_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
             patch("sage_cli.runtime_provider._get_model_config", return_value={"provider": "bedrock"}), \
             patch("sage_cli.runtime_provider.resolve_requested_provider", return_value="auto"), \
    ...
    def test_bedrock_openai_models_route_to_mantle_responses(self, monkeypatch):
        ...
        for model_id in BEDROCK_OPENAI_RESPONSES_MODEL_IDS:
            with patch("sage_cli.runtime_provider.resolve_provider", return_value="bedrock"), \
                 patch("sage_cli.runtime_provider._get_model_config", return_value={
                     "provider": "bedrock",
                     "default": model_id,
                 }):
                result = resolve_runtime_provider(requested="bedrock")
...
class TestProvidersModule:
    """Verify bedrock is wired into sage_cli/providers.py."""
...
class TestAuxiliaryClientBedrockResolution:
    ...
    def test_bedrock_mantle_config_region_beats_env_region(self, monkeypatch):
        ...
        with patch("sage_cli.config.load_config_readonly",
