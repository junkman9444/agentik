...
from sage_cli import lifecycle, plugins
...
def direct_runtime(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.config.read_raw_config_readonly",
...
def test_real_binding_drives_lifecycle_aggregation_export_and_snapshot(
    ...
    monkeypatch.setattr(
        "sage_cli.observability.shared_metrics._utc_now",
        lambda: tomorrow,
    )
...
def test_managed_config_cannot_override_shared_metrics_consent(
    ...
    from sage_cli import config, managed_scope
