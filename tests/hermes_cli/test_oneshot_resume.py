...
class TestRunAgentResumeRuntime:
    ...
    def test_run_agent_uses_stored_runtime_and_reopens_row(self, tmp_path, monkeypatch):
        import sage_cli.oneshot as oneshot_mod
        ...
        monkeypatch.setattr("sage_cli.config.load_config", lambda: {"model": {"default": "ambient-model", "provider": "openrouter"}})
        monkeypatch.setattr(
        ...
        monkeypatch.setattr("sage_cli.tools_config._get_platform_tools", lambda _cfg, _p: [])
        monkeypatch.setattr("sage_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
...
class TestRunOneshotForwardsResume:
    def test_resume_kwarg_reaches_run_agent(self, monkeypatch):
        ...
        monkeypatch.setattr("sage_cli.oneshot._run_agent", _fake_run_agent)
