...
def test_abort_recovery_hands_managed_profiles_to_a_fresh_process(monkeypatch):
    ...
    assert argv[1:4] == ["-m", "sage_cli.update_restart_recovery", "--stdin"]
...
def test_recovery_child_restarts_each_profile_with_a_fresh_main(monkeypatch):
    recovery = importlib.import_module("sage_cli.update_restart_recovery")
    calls = []
    ...
    assert [call[0] for call in calls] == [
        [sys.executable, "-m", "sage_cli.main", "-p", "coder", "gateway", "restart"],
        [sys.executable, "-m", "sage_cli.main", "-p", "default", "gateway", "restart"],
    ]
...
def test_recovery_module_end_to_end_in_a_real_fresh_process(tmp_path):
    ...
    grandchild ``sage_cli.main … gateway restart`` invocations (recording
