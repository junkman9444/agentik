...
from sage_cli.slack_cli import _build_full_manifest, slack_manifest_command
from sage_cli.subcommands.slack import build_slack_parser
from sage_cli import main_platform_setup
...
def _run_console_entrypoint(*argv: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [
            sys.executable,
            "-c",
            "from sage_cli.main import main; raise SystemExit(main())",
            *argv,
        ],
...
def test_slack_dispatcher_propagates_manifest_failure(monkeypatch):
    from sage_cli import main as main_module
    from sage_cli import slack_cli
