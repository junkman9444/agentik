"""Guards for sage_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from sage_cli.config import ...`` silently makes `hermes --version`
...
def test_startup_fast_import_weight():
    ...
    assert not offenders, (
        f"sage_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "sage_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
