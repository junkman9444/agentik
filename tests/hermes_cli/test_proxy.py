...
from sage_cli.proxy.adapters import ADAPTERS, get_adapter
from sage_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from sage_cli.proxy.adapters.nous_portal import NousPortalAdapter
from sage_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_nous_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    with patch(
        "sage_cli.proxy.adapters.nous_portal.resolve_nous_runtime_credentials",
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sage_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
