"""Tests for sage_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                ...
                # sage_constants._get_platform_default_hermes_home.
...
class TestEnsureHermesHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from sage_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("sage_cli.config.sanitize_env_file", return_value=2),
            patch(
                "sage_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("sage_cli.config.read_raw_config", return_value={}),
            patch("sage_cli.config.get_missing_env_vars", return_value=[]),
            patch("sage_cli.config.get_missing_config_fields", return_value=[]),
            patch("sage_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from sage_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
...
class TestConfigSupportFloor:
    ...
    def test_floor_message_uses_display_hermes_home(self):
        from sage_cli.config_migrations import support_floor_message
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from sage_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from sage_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from sage_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from sage_cli.config import _persist_migration, read_raw_config
