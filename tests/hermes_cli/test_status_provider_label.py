...
from sage_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("sage_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("sage_cli.status.resolve_provider", return_value="openrouter"), \
         patch("sage_cli.status.load_config", return_value=config), \
         patch("sage_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("sage_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("sage_cli.status.resolve_provider", return_value="anthropic"):
