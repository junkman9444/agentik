"""Tests for sage_cli.update_receipt — Phase 1 of the fleet-update plan (#91277).
...
def receipt_home(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setattr(
        "sage_cli.config.get_hermes_home", lambda: home, raising=False
    )
...
class TestCommandBoundaryFinalization:
    ...
    def test_cmd_update_boundary_finalizes_on_early_exit(
        ...
        import sage_cli.update_lock as update_lock_mod
...
class TestFleetClassification:
    def _fleet_with(self, monkeypatch, tmp_path, record, expected_sha="a" * 40):
        ...
        monkeypatch.setattr(
            "sage_cli.profiles._get_default_hermes_home", lambda: home
        )
        monkeypatch.setattr(
            "sage_cli.profiles._get_profiles_root",
...
class TestGatewayStatusStamping:
    ...
    def test_code_identity_failure_degrades_to_absent(self, monkeypatch):
        ...
        monkeypatch.setattr("sage_cli.build_info.get_code_identity", _boom)
        record = gs._build_runtime_status_record()
