...
def test_latin1_fallback_stream_honors_override(tmp_path, monkeypatch):
    ...
    from sage_cli.env_loader import _load_dotenv_with_fallback
...
def test_utf32_le_bom_leaves_file_untouched(tmp_path, caplog):
    ...
    from sage_cli.env_loader import _sanitize_env_file_if_needed
    ...
    with caplog.at_level(logging.WARNING, logger="sage_cli.env_loader"):
...
def test_cleanup_scope_is_the_profile_managed_set():
    ...
    """
    from sage_cli.env_loader import _PROFILE_MANAGED_ENV_KEYS
...
# the shared sage_cli.config.apply_terminal_config_to_env helper.
# ---------------------------------------------------------------------------
