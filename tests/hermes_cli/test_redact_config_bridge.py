...
`HERMES_REDACT_SECRETS` at module-import time. `sage_cli/main.py` at
...
env var in `sage_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports sage_cli.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import sage_cli.main  # triggers the bridge + setup_logging
        import agent.redact
