...
from sage_cli.auth import is_runtime_provider_routable
...
def _rows_with_env(monkeypatch, env_name: str, provider: str) -> list[dict]:
    ...
    with (
        ...
        patch("sage_cli.models.cached_provider_model_ids", return_value=["model-a"]),
        patch("sage_cli.providers.HERMES_OVERLAYS", {}),
    ):
