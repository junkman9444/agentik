...
(e.g. orphan merge-conflict markers in ``sage_cli/config.py``), the CLI
...
markers in ``sage_cli/config.py``; users who ran ``hermes update`` in
...
def test_validate_critical_files_syntax_tolerates_missing_files(tmp_path):
    ...
    # Populate everything except sage_constants.py
    for relpath in update_cmd._UPDATE_CRITICAL_FILES:
        if relpath == "sage_constants.py":
            continue
