"""Tests for sage_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `sage_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import sage_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("sage_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from sage_cli.service_manager import _seed_supervise_skeleton
