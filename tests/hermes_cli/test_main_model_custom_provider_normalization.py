...
from sage_cli.web_server_config import _normalize_main_model_assignment


def _normalize(config, provider, model="vendor/model-a"):
    with patch("sage_cli.config.load_config", return_value=config):
...
def test_unknown_vendor_still_uses_aggregator_fallback():
    with patch("sage_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter"):
...
def test_unknown_vendor_without_openrouter_key_is_not_reassigned():
    ...
    with patch("sage_cli.models_detect.provider_has_credentials", lambda p: False):
