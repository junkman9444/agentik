...
def _wire_common(main_mod, monkeypatch):
    monkeypatch.setattr(
        "sage_cli.profiles.get_active_profile_name", lambda: "default"
    )
    ...
    monkeypatch.setattr(
        "sage_cli.mcp_startup.start_background_mcp_discovery",
        lambda **_k: None,
    )
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    ...
    builds = []
    monkeypatch.setattr(
        "sage_cli.main_web_build._build_web_ui", lambda *a, **k: builds.append(a) or True
    )
...
def test_skip_build_missing_dist_attempts_one_recovery_build(
    ...
    dist = project_root / "sage_cli" / "web_dist"
    ...
    monkeypatch.setattr("sage_cli.main_web_build._build_web_ui", fake_build)
