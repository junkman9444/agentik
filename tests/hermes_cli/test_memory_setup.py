...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sage_cli.config.load_config", lambda: {"memory": {}})
...
# behavior is covered by tests/sage_cli/test_memory_setup_env_denylist.py,
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("sage_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "sage_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
