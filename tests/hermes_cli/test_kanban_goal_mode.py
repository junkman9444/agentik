...
class TestCLIJudgeGate:
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from sage_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("sage_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("sage_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("sage_cli.kanban.kbc.connect_closing", fake_connect_closing)
        monkeypatch.setattr("sage_cli.kanban._worker_run_id_for", lambda _: None)
