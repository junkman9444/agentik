...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("sage_cli.setup.ensure_hermes_home", {}),
        ("sage_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("sage_cli.config.is_managed", {"return_value": False}),
        ("sage_cli.setup.load_config", {"return_value": {}}),
        ("sage_cli.setup.save_config", {}),
        ("sage_cli.setup.get_env_value", {"return_value": None}),
        ("sage_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("sage_cli.setup._print_setup_summary", {}),
        ("sage_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("sage_cli.setup.load_config", {"return_value": {}}),
        ("sage_cli.setup.save_config", {}),
        ("sage_cli.auth.get_active_provider", {"return_value": None}),
        ("sage_cli.setup.get_env_value", {"return_value": None}),
        ("sage_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="sage_cli.setup.prompt_choice",
                quick="sage_cli.setup._run_quick_setup",
                model="sage_cli.setup.setup_model_provider",
                terminal="sage_cli.setup.setup_terminal_backend",
                agent="sage_cli.setup.setup_agent_settings",
                gateway="sage_cli.setup.setup_gateway",
                tools="sage_cli.setup.setup_tools",
            )
...
class TestFreshInstall:
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sage_cli.setup.prompt_choice", {"return_value": 0}),
                first="sage_cli.setup_quick._run_first_time_quick_setup",
            )
            from sage_cli.setup import run_setup_wizard
            from sage_cli import setup as setup_mod
    ...
    def test_blank_slate_runs_inside_navigation_step(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("sage_cli.setup.prompt_choice", {"return_value": 2}),
                blank="sage_cli.setup_quick._run_blank_slate_setup",
            )
            from sage_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "sage_cli.setup.run_setup_wizard",
