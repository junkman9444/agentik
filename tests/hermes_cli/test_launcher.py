...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `sage_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "hermes"
    called = []

    fake_main_module = types.ModuleType("sage_cli.main")

    def fake_main():
        called.append("sage_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "sage_cli.main", fake_main_module)
