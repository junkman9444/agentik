...
_GATEWAY_CMD = "python -m sage_cli.main gateway run"
...
class TestProcFallback:
    ...
    def test_detects_gateway_pid_via_proc(self):
        my_pid = os.getpid()
        entries = {
            my_pid: "python -m sage_cli.main",   # own process — excluded
            12345: _GATEWAY_CMD,
            99999: _OTHER_CMD,
        }
        ...
        with (
            ...
            patch("sage_cli.gateway._get_ancestor_pids", return_value=set()),
...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("sage_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("sage_cli.gateway.is_macos", return_value=True),
            patch("sage_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "sage_cli.gateway.get_launchd_label",
                return_value="ai.hermes.gateway.myprofile",
            ),
            patch(
                "sage_cli.gateway._locate_launchd_gateway_service",
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        with (
            ...
            patch(
                "sage_cli.gateway.launchd_gateway_labels_for_install",
