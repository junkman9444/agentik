...
import sage_cli.backup as backup
...
def profiles(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "sage_cli.profiles._get_default_hermes_home", lambda: default_home
    )
    monkeypatch.setattr(
        "sage_cli.profiles._get_profiles_root", lambda: tmp_path / "home" / "profiles"
    )
    monkeypatch.setattr(
        "sage_cli.profiles._PROFILE_ID_RE",
...
class TestSiblingEnumeration:
    ...
    def test_never_raises(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("sage_cli.profiles._get_default_hermes_home", _boom)
