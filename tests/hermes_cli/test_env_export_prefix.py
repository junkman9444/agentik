...
hand-rolled parsers — ``sage_cli.config.load_env``,
``sage_cli.main._has_any_provider_configured``, and
...
def test_config_load_env_strips_export_prefix(tmp_path):
    from sage_cli.config import invalidate_env_cache, load_env
    ...
    try:
        with patch("sage_cli.config.get_env_path", return_value=env_path):
            env = load_env()
    finally:
        invalidate_env_cache()
