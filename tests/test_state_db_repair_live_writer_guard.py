...
def test_repair_refuses_while_another_connection_holds_the_db(tmp_path):
    ...
    (``sage_state_holders.live_writer_holds_db``) first scans for foreign
...
def test_repair_checks_foreign_holders_before_opening_sqlite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sage_state_repair, "_connect_repair_durable", _unexpected_probe)
...
def test_linux_holder_scan_does_not_require_psutil(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sage_state_holders, "psutil", None)

    holders = sage_state_holders.foreign_state_db_holders(
...
def test_incomplete_holder_scan_keeps_unknown_sentinel(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sage_state_holders.os, "readlink", lambda _path: str(db))
    real_stat = sage_state_holders.os.stat
    ...
    holders = sage_state_holders.foreign_state_db_holders(db)
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (["python3", "-m", "sage_cli.main", "gateway"], True),
    ),
)
def test_uninspectable_unknown_descriptor_uses_hermes_identity_at_repair_boundary(
    ...
    if should_block:
        ...
        monkeypatch.setattr(
            sage_state_repair, "_connect_repair_durable",
            _unexpected_probe,
        )
        report = repair_state_db_schema(db, backup=False)
        ...
        real_connect = sage_state_repair._connect_repair_durable
        probe_reached = False
...
def test_uninspectable_watched_identity_blocks_alias_before_sqlite(
    ...
    monkeypatch.setattr(sage_state_holders.os, "listdir", _listdir)
    monkeypatch.setattr(
        sage_state_holders.os, "readlink", lambda _path: str(alias)
    )
    monkeypatch.setattr(sage_state_holders.os, "stat", _stat)
...
def test_uninspectable_alias_descriptor_for_hermes_blocks_before_sqlite(
    ...
    monkeypatch.setattr(
        sage_state_holders,
        "_read_proc_argv",
        lambda _pid: ["python3", "-m", "sage_cli.main", "gateway"],
    )
