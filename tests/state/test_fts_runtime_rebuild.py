...
import sage_state
import sage_state_holders
import sage_state_schema
from sage_state import SessionDB
from sage_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from sage_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m sage_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                ...
                "be-child",
                "ssh",
                "--cmd=python -m sage_cli.main gateway",
            ),
            ...
            ("hermesctl", "serve"),
            ("python3", "worker.py", "sage_cli.main"),
            ("python3", "-m", "other.module", "sage_cli.main"),
            ("python3", "-c", "sage_cli.main"),
            ("python3", "-Icprint('sage_cli.main')", "sage_cli/main.py"),
        ),
    )
    def test_uninspectable_non_hermes_process_is_not_a_holder(self, argv):
        assert not sage_state_holders._looks_like_hermes(argv)
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("/usr/bin/python3", "-m", "sage_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "sage_cli.main", "gateway"),
            ("/usr/bin/python3", "-mhermes_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "sage_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "sage_cli.main"),
            (
                "/opt/hermes-agent/.venv/bin/python",
                "/opt/hermes-agent/sage_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "sage_cli/main.py", "gateway"),
            ("python3", "/opt/hermes-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_hermes_process_remains_a_holder(self, argv):
        assert sage_state_holders._looks_like_hermes(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(sage_state_holders.os, "getpid", lambda: 111)
        real_listdir = os.listdir
        def _listdir(path):
            if isinstance(path, str):
                path = path.replace("/proc", str(proc_root))
            return real_listdir(path)
        monkeypatch.setattr(sage_state_holders.os, "listdir", _listdir)
        real_readlink = os.readlink
        def _readlink(path):
            path = path.replace("/proc", str(proc_root))
            return real_readlink(path)
        monkeypatch.setattr(sage_state_holders.os, "readlink", _readlink)
        real_stat = os.stat
        ...
        monkeypatch.setattr(sage_state_holders.os, "stat", _stat)

        holders = sage_state_holders.foreign_state_db_holders(db_path)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert sage_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(sage_state_holders, "_read_proc_argv", _fake_argv)

        holders = sage_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "sage_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from sage_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(sage_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            sage_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "sage_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(sage_state_schema.time, "time", lambda: clock[0])

        from sage_cli.doctor_state import _render_state_db_stats
        from sage_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(sage_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(sage_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m sage_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + sage_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = sage_state_schema._FTS_STALE_RETRY_MAX_SECONDS
