...
``sage_cli.oneshot``) a bounded linger over every tracked
...
def test_config_reader_falls_back_when_config_unreadable(monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.config.read_raw_config", _boom, raising=False
    )
...
def test_oneshot_module_lingers_before_agent_close():
    """sage_cli/oneshot.py must wait for pending completions before
    agent.close() (which kill_all()s the task's processes)."""
    src = (REPO_ROOT / "sage_cli" / "oneshot.py").read_text(encoding="utf-8")
