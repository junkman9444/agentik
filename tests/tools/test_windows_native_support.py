...
class TestConfigureWindowsStdio:
    """``sage_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("sage_cli.stdio", None)
        # Fresh import now; tests import from sage_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import sage_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("sage_cli.stdio", None)

    def test_no_op_on_posix(self, monkeypatch):
        from sage_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("sage_cli.kanban_db_dispatch", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "sage_cli" / "web_server_chat.py").read_text(encoding="utf-8")
    ...
    def test_pty_handler_checks_availability_flag(self):
        ...
        root = Path(__file__).resolve().parents[2]
        source = (root / "sage_cli" / "web_routers" / "chat_ws.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, sage_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "sage_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call sage_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """sage_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from sage_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from sage_cli._subprocess_compat import resolve_node_command
...
# ---------------------------------------------------------------------------
# sage_cli/kanban_db_dispatch.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "sage_cli" / "kanban_db_dispatch.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / sage_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "sage_cli/tools_config.py",
            "sage_cli/doctor.py",
...
class TestGitBashPathNormalization:
    ...
    def test_posix_noop(self):
        ...
        from sage_cli.worktree_ops import _normalize_git_bash_path
        if sys.platform != "win32":
    ...
    def test_windows_translation(self):
        ...
        """
        from sage_cli import worktree_ops as cli_mod
...
class TestWorktreeSymlinkFallback:
    ...
    def test_source_has_symlink_fallback(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "sage_cli" / "worktree_ops.py").read_text(encoding="utf-8")
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_hermes_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "sage_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "sage_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        sage_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "sage_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from sage_cli._subprocess_compat import" in block
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import sage_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "sage_cli.main", "gateway", "run"]
    ...
    def test_windows_keeps_console_python_and_preserves_tail(self):
        ...
        it up — a pre-import so the lazy ``sage_cli.gateway`` import didn't
        ...
        with mock.patch.object(
            ...
            "sage_cli.config.get_hermes_home", return_value="C:/hermes"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))
