...
on the copy with a devtools port (see sage_cli.browser_connect). These tests
...
class TestRealProfileCdpLaunch:
    ...
    def test_non_chromium_default_fails_closed(self):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("sage_cli.browser_connect.detect_default_chromium", return_value=None):
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
    ...
    def test_snapshot_failure_fails_closed(self):
        self._reset()
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("sage_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sage_cli.browser_connect.snapshot_real_profile", return_value=(None, "boom")):
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
    ...
    def test_launch_returns_http_cdp(self, tmp_path):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("sage_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sage_cli.browser_connect.snapshot_real_profile", return_value=(str(tmp_path), None)), \
             patch("sage_cli.browser_connect.chromium_executable", return_value="/usr/bin/chrome"), \
    ...
    def test_reattaches_to_surviving_chrome_instead_of_overlaying_its_profile(self, tmp_path, live_browser_id):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("sage_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sage_cli.browser_connect.real_profile_copy_dir", return_value=str(tmp_path)), \
             patch("sage_cli.browser_connect.snapshot_real_profile", return_value=(None, "boom")) as snapshot, \
...
class TestConsentConfigRead:
    ...
    def test_consent_read_from_config(self, tmp_path, monkeypatch):
        ...
        with patch("sage_cli.config.read_raw_config",
...
class TestChannelIdentity:
    ...
    def test_channel_sentinel_fails_closed_in_cdp(self):
        ...
        bt._real_profile_cdp_cache.clear()
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             patch("sage_cli.browser_connect.detect_default_chromium",
                   return_value=bc.UNSUPPORTED_CHANNEL), \
             patch("sage_cli.browser_connect.snapshot_real_profile") as snap:
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
...
class TestSnapshotIsCredentialStore:
    ...
    def test_snapshot_dir_secured(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(bc, "get_hermes_home", lambda: tmp_path / "hh")
        called = {"paths": []}
        with patch("sage_cli.config._secure_dir",
...
class TestReviewBugFixes:
    ...
    def test_permissions_secured_on_refresh(self, tmp_path, monkeypatch):
        ...
        bc.snapshot_real_profile("chrome", src=str(src))  # fresh
        secured = []
        with patch("sage_cli.config._secure_dir", side_effect=secured.append):
    ...
    def test_lightpanda_engine_fails_actionably(self):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             ...
             patch("sage_cli.browser_connect.detect_default_chromium") as det:
            cdp, err = bt_real_profile._real_profile_cdp()
        assert cdp is None
...
class TestReviewRound3:
    ...
    def test_consent_off_triggers_cleanup(self, tmp_path, monkeypatch):
        called = {"n": 0}
        with patch.object(bt_cloud, "_use_real_profile", return_value=False), \
             patch("sage_cli.browser_connect.cleanup_real_profile_snapshots",
    ...
    def test_relaunch_path_does_snapshot(self, tmp_path):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
             ...
             patch("sage_cli.browser_connect.detect_default_chromium", return_value="chrome"), \
             patch("sage_cli.browser_connect.real_profile_copy_dir", return_value=str(tmp_path)), \
             patch("sage_cli.browser_connect.snapshot_real_profile",
    ...
    def test_copy_auth_file_bounds_locks_without_overwriting(self, tmp_path, locked):
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-c",
                 "from sage_cli.browser_connect import _copy_auth_file; "
