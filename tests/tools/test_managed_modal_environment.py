...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "sage_cli"}
        or name.startswith("tools.")
        or name.startswith("agent.")
        or name.startswith("sage_cli.")
    }
    try:
        yield
    finally:
        _reset_modules(("tools", "agent", "sage_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    sage_cli = types.ModuleType("sage_cli")
    sage_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["sage_cli"] = sage_cli
    sys.modules["sage_cli.config"] = types.SimpleNamespace(
