...
def test_runtime_inventory_dedupes_same_pid_across_homes(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sage_cli.profiles._get_profiles_root", lambda: profiles_root
    )
    monkeypatch.setattr(
        "sage_cli.gateway._get_service_pids", lambda all_profiles=False: set()
    )
    monkeypatch.setattr(
        "sage_cli.gateway.find_profile_gateway_processes", lambda: []
    )
    monkeypatch.setattr(
        "gateway.control_socket.identify_gateway",
