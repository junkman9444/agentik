...
def _runner_with_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("sage_constants.get_hermes_home", lambda: hermes_home)
    monkeypatch.setattr("sage_cli.config.get_hermes_home", lambda: hermes_home)
...
async def test_warning_enrichment_is_offloaded(tmp_path, monkeypatch):
    ...
    from sage_cli import context_switch_guard
