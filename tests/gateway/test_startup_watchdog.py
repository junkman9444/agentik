...
top-level module ``sage_startup_watchdog``), disarmed once the gateway's asyncio loop is confirmed
...
import sage_startup_watchdog as sw
...
class TestContracts:
    ...
    def test_implementation_module_is_stdlib_only(self):
        ...
        must not import the gateway/agent/sage_cli graphs at module level."""
        import ast
        import inspect
    ...
    def test_all_documented_entry_points_arm_the_watchdog(self):
        ...
        arm_sites = {
            ...
            "sage_cli/main.py": "argv fast-path (standard `hermes gateway run`)",
            "sage_cli/gateway.py": "run_gateway() config-bridge re-arm",
        ...
        for rel, reason in arm_sites.items():
            ...
            aliases = set()
            for node in ast.walk(tree):
                if isinstance(node, ast.ImportFrom) and node.module == "sage_startup_watchdog":
...
class TestProgressLease:
    ...
    def test_schema_init_declares_lease(self):
        """sage_state_schema._init_schema must hold a progress lease so
        multi-GB migrations aren't misread as deadlocks (wiring contract)."""
        import inspect

        import sage_state_schema

        src = inspect.getsource(sage_state_schema.SessionSchemaMixin._init_schema)
    ...
    def test_repair_declares_lease(self):
        ...
        src = inspect.getsource(sage_state_repair.repair_state_db_schema)
        assert "report_startup_progress" in src
