"""The conftest WAL gate must agree with sage_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``sage_state``. That is deliberate: importing
``sage_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
...
def test_conftest_gate_agrees_with_hermes_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and sage_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_hermes_state_at_collection():
    """The gate must stay import-free of sage_state.
    ...
    the gate must work with ``sage_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "sage_state" or name.startswith("sage_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported sage_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.hermes during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("sage_state", None)
