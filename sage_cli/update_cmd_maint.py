...
Split out of ``update_cmd.py``, which re-imports every name so ``sage_cli.update_cmd.<name>``
...
from sage_cli.update_cmd_common import _best_effort

# Log-record parity with the origin module.
logger = logging.getLogger("sage_cli.update_cmd")


_UPDATE_RUNTIME_RELOAD_MODULES = "sage_constants", "tools.environments.local", "tools.lazy_deps"
...
_STALE_PURGE_PREFIXES = "sage_cli", "gateway", "tools", "tui_gateway", "agent"
...
_STALE_PURGE_PROTECTED = frozenset({"sage_cli", "sage_cli.main", "sage_cli.sage_logging"})
...
_STALE_PURGE_PROTECTED_PREFIX = "sage_cli.update_"
...
def _purge_stale_hermes_modules() -> None:
    ...
    """
    from sage_cli.update_cmd import _m
...
def _reload_process_scan_modules() -> None:
    ...
    ``_scan_dashboard_processes`` does a function-level ``from sage_cli._subprocess_compat import
    ...
    """
    _reload_modules(
        ("sage_cli._subprocess_compat", "sage_cli.dashboard_procs"),
        modules=sys.modules,
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    See #83595.
    """
    from sage_cli.update_cmd import _m, _reload_process_scan_modules
    if node_failures:
        print()
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from sage_cli.update_cmd import _branch_head_suffix
...
def _post_update_sqlite_runtime_status():
    ...
    from sage_cli.update_cmd import _m
    from sage_constants import project_venv_dir
    from sage_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    ...
    from sage_cli.update_cmd import _post_update_sqlite_runtime_status
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from sage_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from sage_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from sage_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from sage_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    See #97994.
    """
    from sage_cli.update_cmd import get_hermes_home
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from sage_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from sage_cli.backup import create_pre_update_snapshots_all_profiles
        ...
        if _sibling_snaps:
            ...
            import sage_cli.update_cmd_config as _cfg
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in sage_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    with suppress(Exception):
        from sage_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from sage_cli.profiles import backfill_profile_envs
...
def _print_plugin_compat_notice() -> None:
    ...
    from sage_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from sage_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from sage_cli._install_repair import migrate_windows_bin_path
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from sage_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    try:
        # See #95596.
        from sage_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from sage_cli.model_catalog import seed_cache_from_checkout
