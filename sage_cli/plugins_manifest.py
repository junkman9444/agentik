...
Split out of :mod:`sage_cli.plugins`; validation warns and never fails a load.
"""
...
def _plugins_debug() -> bool:
    from sage_cli import plugins as _origin
...
def running_hermes_version() -> str:
    """Installed ``hermes-agent`` distribution version, else ``sage_cli.__version__`` (source checkout)."""
    try:
