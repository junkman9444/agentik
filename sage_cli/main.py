...
# sage_bootstrap must be the very first import — it sets up UTF-8 stdio on
...
from sage_cli._subprocess_compat import suppress_platform_ver_console
...
# Inline path math so ``python sage_cli/main.py`` (script mode: sys.path[0]
# is sage_cli/, not the repo root) can import sage_cli._startup_fast.
_bootstrap_root = os.path.realpath(os.path.join(os.path.dirname(__file__), os.pardir))
if _bootstrap_root not in sys.path:
    sys.path.insert(0, _bootstrap_root)
from sage_cli import _startup_fast  # noqa: E402
...
# the sage_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``sage_startup_watchdog`` is stdlib-only so it
...
if _argv_is_gateway_run(sys.argv[1:]):
    try:
        from sage_startup_watchdog import arm_startup_watchdog as _arm_sw
...
def _run_and_exit_oneshot(
    ...
    *,
    ...
    try:
        from sage_cli.oneshot import run_oneshot
...
# sage_cli.config is importable. Cached so early callers don't re-parse YAML.
...
from sage_cli.subcommands.sync import build_sync_parser
from sage_cli.subcommands.gateway import build_gateway_parser
from sage_cli.subcommands.profile import build_profile_parser
from sage_cli.subcommands.model import build_model_parser
from sage_cli.subcommands.setup import build_setup_parser

from sage_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
...
from sage_cli.subcommands.pause import build_pause_parser
...
from sage_cli.subcommands.verify import build_verify_parser
from sage_cli.subcommands.security import build_security_parser
from sage_cli.subcommands.approvals import build_approvals_parser
...
from sage_cli.subcommands.import_agent import build_import_agent_parser
from sage_cli.subcommands.config import build_config_parser
from sage_cli.subcommands.skin import build_skin_parser
from sage_cli.subcommands.console import build_console_parser
...
from sage_cli.subcommands.memory import build_memory_parser
from sage_cli.subcommands.acp import build_acp_parser
from sage_cli.subcommands.tools import build_tools_parser
from sage_cli.subcommands.insights import build_insights_parser
from sage_cli.subcommands.monitoring import build_monitoring_parser
from sage_cli.subcommands.skills import build_skills_parser
from sage_cli.subcommands.pairing import build_pairing_parser
from sage_cli.subcommands.plugins import build_plugins_parser
from sage_cli.subcommands.mcp import build_mcp_parser
from sage_cli.subcommands.claw import build_claw_parser
from sage_cli.subcommands.vault import build_vault_parser
...
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors sage_cli.profiles._PROFILE_ID_RE
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    """
    from sage_cli._parser import top_level_value_flag_sets
...
def _apply_profile_override() -> None:
    ...
    if profile_name is None:
        return
    try:
        from sage_cli.profiles import resolve_profile_env
...
# app's ``python -m sage_cli.main`` spawn. Gates fail toward inaction. Sits
...
if sys.platform == "win32":
    try:
        from sage_cli import _install_repair as _install_repair_mod
...
# Bridge security.redact_secrets → HERMES_REDACT_SECRETS BEFORE sage_logging
...
try:
    ...
    # sage_logging and later raw reads: 3-4 config.yaml parses become one.
    from sage_cli.config import read_raw_config as _read_raw_early
...
try:
    from sage_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from sage_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if sage_constants not importable yet
...
from sage_cli.model_setup_flows import (
...
from sage_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from sage_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from sage_cli.main_dashboard import (
...
from sage_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _respawn_dashboard_processes,
)
from sage_cli.main_provider_setup import (
...
from sage_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from sage_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from sage_cli.main_desktop import (
    cmd_gui,
)
from sage_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _desktop_build_needed,
    _desktop_dist_exists,
    _desktop_macos_relaunchable_fixup,
    _desktop_packaged_executable,
)
from sage_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from sage_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from sage_cli.main_tui_launch import (
    _launch_tui,
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    counts only once its identity exists, and the boot bootstrap (``sage_cli.free_tier_bootstrap``)
    ...
    """
    from sage_cli.config import DEFAULT_CONFIG, get_env_path, get_hermes_home, load_config
    from sage_cli.auth import PROVIDER_REGISTRY, get_auth_status
    ...
    try:
        from sage_cli.anon_auth import guest_enabled, has_guest
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from sage_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a ``SessionDB`` (lazy import, so test patches on ``sage_state``
...
def _resolve_continue_arg(args, *, use_tui: bool) -> None:
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            try:
                from sage_cli.terminal_breadcrumbs import resolve_breadcrumb_session
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from sage_cli.foreign_sessions import import_foreign_session, pick_foreign_session
...
def _warn_retired_xai_models() -> None:
    ...
    try:
        from sage_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from sage_cli.config import load_config as _load_config_for_xai_check
...
def _start_chat_background_prefetch() -> None:
    ...
    if _termux_should_prefetch_update_check():
        try:
            from sage_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import sage_cli.banner as _banner_mod
...
def cmd_chat(args):
    ...
    try:
        ...
        # Mixed-version installs (new cli.py, older sage_cli.config) crash
...
def cmd_proxy(args):
    ...
    from sage_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
cmd_setup = _forward_command("cmd_setup", "sage_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "sage_cli.auth", "login_command", doc='Authenticate Hermes CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "sage_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "sage_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "sage_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "sage_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "sage_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "sage_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "sage_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "sage_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "sage_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "sage_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "sage_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "sage_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "sage_cli.backup", "run_import", doc='Restore a Hermes backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "sage_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nous Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "sage_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "sage_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "sage_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "sage_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "sage_cli.mcp_config", "mcp_command")
cmd_claw = _forward_command("cmd_claw", "sage_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "sage_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from sage_cli.models import clear_provider_models_cache
            clear_provider_models_cache()
            print("  Cleared model picker cache.")
        except Exception:
            pass
    from sage_cli.setup import run_setup_action_with_navigation
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from sage_cli.auth import AuthError, format_auth_error, resolve_provider
    from sage_cli.config import get_compatible_custom_providers, get_env_value
    from sage_cli.providers import custom_provider_aliases, resolve_provider_full
...
def select_provider_and_model(args=None):
    ...
    from sage_cli.models import _PROVIDER_LABELS
    ...
    # sage_cli.main keep intercepting.
...
# ``sage_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on sage_cli.main; importing update_cmd eagerly would cost every ``hermes``
...
def cmd_verify(args):
    ...
    from sage_cli.verify_cmd import run_verify_command
...
def cmd_security(args):
    ...
    if sub in ("audit", None):
        from sage_cli.security_audit import cmd_security_audit
...
def cmd_approvals(args):
    ...
    from sage_cli.approvals_suggest import approvals_command
...
def cmd_config(args):
    ...
    from sage_cli.config import config_command
...
def cmd_uninstall(args):
    ...
    if getattr(args, "gui_summary", False):
        from sage_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from sage_cli.uninstall import run_gui_uninstall
    ...
    from sage_cli.uninstall import run_uninstall
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from sage_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    from sage_cli.config import is_managed, managed_error
    ...
    from sage_cli.update_contract import (
...
def cmd_update(args):
    ...
    from sage_cli.update_lock import (
    ...
    from sage_cli.update_cmd import _cmd_update_impl
...
from sage_cli.profile_cmd import cmd_profile
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    try:
        from sage_logging import setup_logging as _setup_logging_gui
    ...
    try:
        ...
        # #61115, #65696.
        from sage_cli.config import apply_terminal_config_to_env
...
def cmd_dashboard(args):
    ...
    from sage_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from sage_cli import completion
...
def cmd_logs(args):
    ...
    from sage_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    ...
    from sage_cli.console_engine import run_console_repl
...
def _prepare_agent_startup(args) -> None:
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from sage_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from sage_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from sage_cli.mcp_startup import get_mcp_server_filter
...
def _try_fast_serve_launch() -> bool:
    ...
    try:
        from sage_cli.config import get_container_exec_info
...
def _cmd_sessions_lazy(args, **kwargs):
    ...
    from sage_cli.sessions_cmd import cmd_sessions
...
def _build_cli_parser():
    ...
    live in ``sage_cli/subcommands/<group>.py`` with handlers injected so
    ...
    from sage_cli.send_cmd import register_send_subparser
    ...
    from sage_cli.subcommands.peer import build_peer_parser
    build_peer_parser(subparsers)

    from sage_cli.kanban import build_parser as _build_kanban_parser
    _build_kanban_parser(subparsers).set_defaults(func=cmd_kanban)

    from sage_cli.projects_cmd import build_parser as _build_project_parser
...
def main():
    ...
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from sage_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('sage_cli.cli_output', 'line_input'),
}
