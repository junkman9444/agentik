...
import asyncio
import contextlib
from sage_cli.cli_output import line_input
import json
import logging
import os
import shlex
...
# display_hermes_home is imported lazily: sage_constants may be a cached pre-update version.
...
def _windows_process_listing() -> str | None:
    ...
    from sage_cli._subprocess_compat import bounded_probe_run
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "sage_cli.main"]
    if profile != "default":
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from sage_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    if sys.platform == "win32":
        try:
            from sage_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        # HERMES_HOME) the windowless base interpreter needs to import sage_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    ...
    if not unit_home:
        ...
        unit_home = _parse_kv_pairs(env_line.split()).get("SAGE_HOME", "").strip()
    if unit_home and os.environ.get("SAGE_HOME", "").strip() != unit_home:
        os.environ["SAGE_HOME"] = unit_home
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from sage_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    try:
        from sage_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from sage_cli.gateway_windows import get_task_name  # profile-aware task name
...
from sage_constants import is_container, is_termux, is_wsl
...
def _gw_windows():
    """Lazily import :mod:`sage_cli.gateway_windows` (Windows-only service backend)."""
...
def _profile_suffix() -> str:
    ...
    bare one."""
    import hashlib
    from sage_constants import _get_platform_default_hermes_home, get_default_hermes_root
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # Update-boundary: a gateway restarted mid-update can hold a stale sage_constants
            # without this symbol; see _reload_hermes_constants() in sage_cli/managed_uv.py.
            from sage_cli.managed_uv import _reload_hermes_constants
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    ...
    current_hermes_raw = os.environ.get("SAGE_HOME", "").strip()
    ...
    current_default = Path.home() / ".sage"
    target_default = Path(target_home_dir) / ".sage"
    try:
...
def _append_node_dir_for_service(path_entries: list[str], hermes_root: Path | None = None) -> None:
    ...
    from sage_constants import (hermes_managed_node_tree_present, iter_hermes_node_dirs)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        target_root = Path(home_dir) / ".sage"
        try:
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m sage_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m sage_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "sage_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    return [get_python_path(), "-m", "sage_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from sage_cli.resource_limits import configured_nofile_soft_limit
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from sage_startup_watchdog import (
        ...
        try:
            from sage_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from sage_constants import get_hermes_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
            line = {
...
def _respawn_storm_backoff() -> None:
    ...
    try:
        ...
        if _storm is not None:
            ...
            try:
                from sage_startup_watchdog import kick_startup_watchdog
...
def _setup_standard_platform(platform: dict):
    ...
    from sage_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _running_under_s6() -> bool:
    from sage_cli.service_manager import detect_service_manager
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the sage_cli.setup cycle."""
    from sage_cli import setup as _s
    return {
...
def _dispatch_via_service_manager_if_s6(action: str, profile: str | None = None) -> bool:
    ...
    from sage_cli.service_manager import (
