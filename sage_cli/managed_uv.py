...
from sage_cli.sqlite_runtime import (
...
def _reload_hermes_constants():
    """Re-execute ``sage_constants`` from disk (the imported one may predate venv_python_path)."""
    import sage_constants
    return importlib.reload(sage_constants)
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("sage_cli.main")
...
def _repair_windows_preflight(
    ...
    if self_locked:
        ...
        for line in (
            ...
            f"      cd {root}",
            "      <system Python> -m sage_cli.main update",
