...
:mod:`sage_cli.runtime_provider`; origin-internal collaborators
...
from sage_cli.providers import custom_provider_aliases, custom_provider_slug
from utils import base_url_hostname

logger = logging.getLogger("sage_cli.runtime_provider")

_LLAMACPP_ALIASES = ("llamacpp", "llama.cpp", "llama-cpp")


def _rp():
    """Origin module, late-bound so test patches on ``sage_cli.runtime_provider.*`` apply."""
    import sage_cli.runtime_provider as origin
    return origin
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from sage_cli.models import normalize_opencode_base_url, opencode_model_api_mode
