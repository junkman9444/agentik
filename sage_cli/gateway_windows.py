...
def _current_profile_cli_args() -> list[str]:
    ...
    from sage_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "sage_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from sage_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m sage_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
    argv = [python_exe, "-m", "sage_cli.main"]
    if profile_arg:
...
def _launcher_settings() -> tuple[str, str, str, str]:
    ...
    from sage_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, hermes_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('SAGE_HOME')}) = {q(hermes_home)}",
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from sage_cli.gateway import PROJECT_ROOT
    ...
    env_overlay = {"SAGE_HOME": hermes_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_hermes_home_path(venv_dir)}
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    if hermes_home:
        env_overlay["SAGE_HOME"] = hermes_home
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    from sage_cli.setup import prompt_yes_no
