...
def _scrub_config_yaml_mirrors(old_value: str, new_value: str | None) -> List[str]:
    ...
    from sage_cli.config import get_config_path, require_readable_config_before_write
...
def purge_env_credential_references(
    ...
    _for_each_provider(providers, "sage_cli.auth.suppress_credential_source", f"env:{env_var}")
    if clear_models_cache and providers:
        # Best-effort — a cache failure must not block the credential removal itself.
        _for_each_provider(providers, "sage_cli.models.clear_provider_models_cache")
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    """
    from sage_cli.config import load_env, save_env_value
    ...
    _for_each_provider(providers, "sage_cli.auth.unsuppress_credential_source", f"env:{env_var}")
...
def remove_provider_env_credential(env_var: str) -> Dict[str, Any]:
    ...
    from sage_cli.config import load_env, remove_env_value
