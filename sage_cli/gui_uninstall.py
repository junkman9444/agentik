...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    from sage_cli.linux_desktop_entry import desktop_entry_path
...
def agent_is_installed(hermes_home: Path) -> bool:
    ...
    return any((hermes_home / "hermes-agent" / sub).is_dir() for sub in ("sage_cli", "venv", ".venv"))
...
def uninstall_gui(hermes_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        ...
        try:
            from sage_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
