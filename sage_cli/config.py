...
from sage_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from sage_cli.secret_prompt import masked_secret_prompt
# Re-export from sage_constants — canonical definition lives there.
from sage_constants import get_hermes_home, get_process_hermes_home  # noqa: F401
...
def _warn_config_parse_failure(
    ...
    from sage_cli.config_backups import backup_config
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "SAGE_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    """
    # The stamp is a property of the running code tree (parent of sage_cli/), NOT of $HERMES_HOME,
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``sage_constants.is_container``."""
    try:
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (sage_cli/main.py) so the wording never
...
def ensure_hermes_home():
    ...
    from sage_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _HERMES_HOME_ENSURED and home.is_dir():
        return
    from sage_cli.config_home import initialize_home
    initialize_home(home, _HERMES_HOME_SUBDIRS, _HERMES_HOME_ENSURED)


# ---- Config loading/saving ----

from sage_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from sage_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use sage_cli.config.<name>)
...
# Back-compat re-exports — :mod:`sage_cli.personality` owns personality/overlay semantics.
from sage_cli.personality import (  # noqa: E402,F401
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (sage_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # sage_cli/tools_config.py toolset-save flow
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from sage_cli.config_home import config_load_issue
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from sage_cli.toolset_scope import toolset_allowed_for_platform
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from sage_cli.credential_lifecycle import save_provider_env_credential
...
def show_config():
    ...
    print(color("│                  ⚕ Sage Configuration                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from sage_cli.credential_lifecycle import remove_provider_env_credential
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``sage_cli/`` — i.e. the git checkout for source
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('sage_cli.route_identity', 'normalize_route_base_url'),
}
