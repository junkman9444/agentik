...
from sage_cli.subcommands._shared import add_accept_hooks_flag
...
def build_cron_parser(subparsers, *, cmd_cron: Callable) -> None:
    ...
    cron_create.add_argument("--script",
        help="Path to a script under ~/.sage/scripts/. Default mode: "
    ...
    cron_create.add_argument("--monitor-script", dest="monitor_script",
        help="Monitor mode: path to a cheap source script under "
            "~/.sage/scripts/ that runs each tick BEFORE the agent. "
    ...
    cron_edit.add_argument("--script",
        help="Path to a script under ~/.sage/scripts/. Pass empty string to clear. "
