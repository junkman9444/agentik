"""Dashboard process-hygiene helpers — extracted from ``sage_cli/main.py``.

Helpers defined in ``sage_cli.main_dashboard`` / ``sage_cli.main_install_repair`` are imported at
...
_DASHBOARD_PATTERNS = tuple(
    ...
    for launcher in ("hermes", "sage_cli.main", "sage_cli/main.py"))
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Hermes
    ...
    with contextlib.suppress(Exception):
        ...
        from sage_cli.process_identity import ledger_entries
...
def _hermes_home_for_pid(pid: int) -> str | None:
    ...
    with contextlib.suppress(Exception):
        import psutil
        if home := psutil.Process(pid).environ().get("SAGE_HOME"):
            return home
    try:
...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    """
    from sage_cli import main_dashboard as _dash
...
def _detect_concurrent_hermes_instances(
    ...
    *shim* ancestor (the setuptools launcher is a separate native process from its
    ...
    """
    from sage_cli.main_install_repair import _hermes_exe_shims, _is_windows
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    """
    cmd = command.lower()
    if "serve" not in cmd or ("hermes" not in cmd and "sage_cli" not in cmd):
        return False
...
def _hermes_home_dir() -> Path:
    ...
    override = os.environ.get("SAGE_HOME", "").strip()
    return Path(override).expanduser() if override else Path.home() / ".sage"
