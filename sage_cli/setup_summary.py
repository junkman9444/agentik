...
resolved through the module object so test patches on ``sage_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from sage_cli import nous_compat as nous_subscription

logger = logging.getLogger("sage_cli.setup")
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'sage setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'sage setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'sage tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("sage setup", "             Re-run the full wizard"), ("sage setup model", "       Change model/provider"),
    ("sage setup terminal", "    Change terminal backend"), ("sage setup gateway", "     Configure messaging"),
    ("sage setup tools", "       Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("sage config", "           View current settings"), ("sage config edit", "      Open config in your editor"),
    ("sage config set <key> <value>", ""))
_READY_ROWS = (
    ("sage", "                Start chatting"), ("sage gateway", "        Start messaging gateway"),
    ("sage doctor", "         Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'sage setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `sage tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'sage setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via sage auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Sage cannot chat yet.")
        _setup._info("  Finish this one step with:",
              "    sage model              (pick any provider/model)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'sage setup tools' to configure them,")
...
import sage_cli.setup as _setup  # noqa: E402  (bottom: sage_cli.setup imports this module)
