"""Dependency-light venv recovery that runs BEFORE sage_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``sage_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``sage_cli.main`` to become importable again (force-reinstall of
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``sage_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    """Repair wiped core packages so ``sage_cli.main`` can import at all.
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    venv extension mapped; this process, running before ``sage_cli.main``'s third-party imports, maps
