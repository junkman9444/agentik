...
Re-exported from ``sage_cli/auth.py`` (patch targets unchanged); origin helpers are imported
lazily per function so ``sage_cli.auth.<helper>`` patches still intercept and no cycle forms.
"""
...
def _spotify_token_payload_to_state(
    ...
    from sage_cli.auth import _coerce_ttl_seconds
...
def resolve_spotify_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    from sage_cli.auth import _auth_store_lock, _is_expiring, _load_auth_store, _load_provider_state, _quarantine_flat_oauth_state, _refresh_spotify_oauth_state, _save_auth_store, _store_provider_state
...
def get_spotify_auth_status() -> Dict[str, Any]:
    from sage_cli.auth import _is_expiring, get_provider_auth_state
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    from sage_cli.auth import _is_remote_session
...
def login_spotify_command(args) -> None:
    from sage_cli.auth import _auth_store_lock, _can_open_graphical_browser, _is_remote_session, _load_auth_store, _print_loopback_ssh_hint, _save_auth_store, _store_provider_state, get_provider_auth_state
