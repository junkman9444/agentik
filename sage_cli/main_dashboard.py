...
Split out of ``sage_cli/main.py``. Names that still live in main (``PROJECT_ROOT``, ...)
...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None) -> list[int]:
    ...
    from sage_cli.dashboard_procs import _scan_dashboard_processes
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"hermes {candidate}", f"sage_cli.main {candidate}", f"sage_cli/main.py {candidate}")
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        # sage_cli.config.get_hermes_home to simulate setup failure.
        from sage_cli.config import get_hermes_home as _get_hermes_home
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    try:
        from sage_cli.web_server import should_require_dashboard_auth
    ...
    try:
        ...
        from sage_cli.plugins_cmd import ensure_basic_auth_plugin_enabled_in_config
        cfg = load_config()
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    if sys.platform == "win32":
        from sage_cli.windows_ssh_runtime import read_token
    ...
    # See #69551.
    token_root = Path.home() / ".sage" / "desktop-ssh"
    try:
...
def _route_named_profile_dashboard(
    ...
    reexec_argv = [
        sys.executable, "-m", "sage_cli.main",
        "-p", "default",
    ...
    try:
        ...
        env["SAGE_HOME"] = str(get_default_hermes_root())
    except Exception:
        env.pop("SAGE_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    """
    from sage_cli.main import PROJECT_ROOT
    from sage_cli.main_web_build import _build_web_ui
    ...
    if _headless_backend:
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "sage_cli" / "web_dist"
        )
