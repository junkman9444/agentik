...
DeepInfra pricing adapters. Split out of ``sage_cli.models``; helpers still defined there are
looked up on ``sage_cli.models`` at call time so ``patch("sage_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from sage_cli.models_reasoning_caps import _seed_reasoning_caps
...
def nous_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    ...
    try:
        from sage_cli.nous_compat import nous_policy_present
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from sage_cli.models import _pricing_profile_key
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from sage_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from sage_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
