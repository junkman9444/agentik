...
from sage_cli.config import get_hermes_home, get_config_path, load_config, save_config
from sage_constants import get_optional_skills_dir
from sage_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          ⚕ Sage — {title:<37s}│", f"└{rule}┘"):
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    if not opts.no_backup:
        try:
            from sage_cli.backup import create_pre_migration_backup
