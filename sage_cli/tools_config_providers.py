...
from sage_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
...
logger = logging.getLogger("sage_cli.tools_config")
...
def _visible_providers(
    ...
    from sage_cli.tools_config import get_nous_subscription_features
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from sage_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    from sage_cli.tools_config import _prompt_choice, _provider_env_ready, get_nous_subscription_features
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from sage_cli.tools_config import _provider_env_ready
...
def _pick_model_from_catalog(
    ...
    selection."""
    from sage_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    ...
    from sage_cli.tools_config import _cfg_section
...
def _nous_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    ...
    if managed_feature:
        from sage_cli.nous_compat import ensure_nous_portal_access
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    ...
    from sage_cli.tools_config import TOOL_CATEGORIES, get_nous_subscription_features
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    from sage_cli.tools_config import _prompt_choice
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from sage_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
