...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from sage_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`sage_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
...
def _append_unconfigured_rows(
    ...
    from sage_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from sage_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _apply_pricing(rows: list[dict], *, force_fresh_nous_tier: bool = False, cached_only: bool = False) -> None:
    ...
    from sage_cli.models_pricing import (
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from sage_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from sage_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from sage_cli.models_pricing import pricing_cache_scope
