...
from sage_cli.doctor import _section, check_info
from sage_cli.doctor_report import check_fail, check_ok, check_warn
...
def _browser_available() -> bool:
    ...
    if shutil.which("agent-browser"):
        return True
    try:
        from sage_cli.doctor import HERMES_HOME, PROJECT_ROOT
    ...
    # probes above. Mirror the rung sage_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    from sage_cli.mcp_config import _probe_single_server
