...
from sage_cli.plugin_capabilities import _child_dict
...
def _read_manifest(plugin_dir: Path) -> dict:
    ...
    if manifest_file is None:
        if not _has_portable_manifest(plugin_dir):
            return {}
        try:
            from sage_cli.agent_plugins import read_agent_plugin_manifest
...
def cmd_install(
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from sage_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
...
def cmd_update(name: str) -> None:
    ...
    if declared_caps:
        from sage_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from sage_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from sage_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from sage_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from sage_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    ...
    from sage_cli.plugin_capabilities import (
        CAPABILITY_REGISTRY,
...
def _discover_all_plugins() -> list:
    ...
    from sage_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    try:
        ...
        pass
    try:
        from sage_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from sage_cli.curses_ui import _addnstr, flush_stdin
...
def _run_plugin_git(
    ...
    if auth_url:
        from sage_cli.git_credentials import with_git_auth
...
def _catalog():
    from sage_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from sage_cli.plugin_packs import pack_command
    pack_command(args)
...
def cmd_compat(args: Any | None = None) -> None:
    ...
    import sys
    from pathlib import Path
    from sage_cli.plugin_compat import (
