...
Split out of ``sage_cli/auth.py``; origin helpers are imported lazily inside each function
so ``sage_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
# Log-record parity with the origin module (caplog tests pin "sage_cli.auth").
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from sage_cli.auth import _POOL_STATUS_FIELDS
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from sage_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    from sage_cli.auth import _load_provider_state, _nonempty_str
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    ...
    from sage_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def _codex_refresh_failure_error(response: "httpx.Response") -> AuthError:
    ...
    from sage_cli.auth import _nonempty_str
...
def refresh_codex_oauth_pure(
    ...
    from sage_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    from sage_cli.auth import _save_codex_tokens, refresh_codex_oauth_pure
    try:
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from sage_cli.auth import _codex_access_token_is_expiring
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from sage_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from sage_cli.auth import _utc_now_z
