...
The identity is created in exactly one place, at boot (``sage_cli.free_tier_bootstrap``), and only
...
def current_nous_state() -> Optional[Dict[str, Any]]:
    ...
    from sage_cli.auth import _load_auth_store, _load_provider_state
    try:
...
def apply_exchange_to_state(state: Dict[str, Any], exchanged: Dict[str, Any]) -> None:
    ...
    from sage_cli.auth import _validate_nous_inference_url_from_network
...
def _portal_base_url() -> str:
    from sage_cli.auth import _nous_portal_env_override
...
def _mint_locked(
    ...
    from sage_cli.auth import _store_provider_state, _save_auth_store
    from sage_cli.auth import _write_shared_nous_state
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    from sage_cli.auth import _nous_portal_base_url
...
def clear_dead_guest(reason: str, *, dead_token: Optional[str] = None) -> None:
    ...
    *dead_token* is None the profile's current guest is treated as the failed one.
    ...
    from sage_cli.auth import _clear_shared_nous_state, _nous_shared_store_lock, _read_shared_nous_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    try:
        ...
        if str(model_cfg.get("default") or "").strip() == requested:
            from sage_cli.auth import _update_config_for_provider
...
def _account_state_from_token(
    ...
    from sage_cli.auth import PROVIDER_REGISTRY, _coerce_ttl_seconds, _optional_base_url, _tls_state_from_verify
    from sage_cli.auth import _NOUS_EMPTY_AGENT_KEY_FIELDS, _iso_after, refresh_nous_oauth_from_state
    now = datetime.now(timezone.utc)
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    default for the account's tier (:func:`sage_cli.models.recommended_nous_default_model`, the
    ...
    model = current
    if on_welcome_model:
        from sage_cli.models import recommended_nous_default_model
        try:
...
def _poll_for_token(*args, **kwargs) -> Dict[str, Any]:
    ...
    from sage_cli.auth_device_flow import _poll_for_token as poll
...
def persist_nous_credentials(*args, **kwargs):
    ...
    from sage_cli.auth import persist_nous_credentials as persist
...
from sage_cli.anon_sign_in import (  # noqa: E402
...
from sage_cli.anon_sign_in_cli import (  # noqa: E402
