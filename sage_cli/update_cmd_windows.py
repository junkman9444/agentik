...
Split out of ``update_cmd.py``; names are re-imported there so ``sage_cli.update_cmd.<name>`` still resolves/monkeypatches.
...
logger = logging.getLogger("sage_cli.update_cmd")  # log-record parity with the origin module
...
def _detect_venv_python_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str, str]]:
    ...
    for proc in proc_iter:
        ...
        # its .pyd files — match cmdline (venv path, or `-m sage_cli.main` + root/cwd).
        if in_venv or venv_prefix in cmdline_low or (
            "sage_cli.main" in cmdline_low and (root_prefix in cmdline_low or _cwd_prefix(proc).startswith(root_prefix))
        ):
...
def _hermes_holder_subcommand(cmdline: str) -> str | None:
    ...
    Token-based, never substring (``kanban --preserve-cache`` contains "serve"): find the ``sage_cli.main`` /
    ...
    def _is_entry(i: int, token: str) -> bool:
        ...
        return (low.endswith("sage_cli.main") and i > 0 and tokens[i - 1] == "-m") or (
...
def _leftover_pausable_gateway_pids(matches: list[tuple[int, str, str]]) -> list[int] | None:
    ...
    from sage_cli._scan_venv_blockers import _is_pausable_gateway
    psutil = _psutil()
    pids: list[int] = []
...
def _ledger_manual_serve_holders(matches: list[tuple[int, str, str]]) -> list[dict]:
    ...
    try:
        from sage_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    from sage_cli.update_cmd import _m, _record_update_step
...
def _is_backend_argv(argv_low: str) -> bool:
    """Whether a lower-cased argv is a Desktop backend (``sage_cli.main`` running ``serve``/``dashboard``)."""
    return "sage_cli.main" in argv_low and (" serve" in argv_low or " dashboard" in argv_low)
...
def _ledger_reapable_backend_pids(matches: list[tuple[int, str, str]]) -> list[int]:
    ...
    try:
        from sage_cli.process_identity import REAPABLE_PURPOSES, ledger_entries, spawner_is_dead
...
def _stop_process_trees(pids: list[int] | list[tuple[int, int]]) -> None:
    ...
    from sage_cli._subprocess_compat import pid_is_hermes, windows_hide_flags
    for entry in pids:
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    ...
    """
    return "sage_cli.main" in (cmdline or "").lower() and _hermes_holder_subcommand(cmdline) in _BACKEND_PURPOSES
...
def _restore_windows_gateway_service(name: str, *, timeout: float = 60.0) -> None:
    ...
    from sage_cli.update_cmd import _start_windows_gateway_service
    service = _win_service(name)[1]
...
def _windows_cold_start_plan() -> dict | None:
    ...
    from sage_cli.update_cmd import _desktop_owns_gateway_lifecycle
...
def _pause_windows_gateway_services(service_gateways, token: dict, profiles: dict, unmapped: list) -> dict:
    ...
    from sage_cli.update_cmd import _restore_windows_gateway_service, _stop_windows_gateway_service
    paused_services = []
...
def _discover_windows_gateways():
    ...
    from sage_cli.gateway import find_gateway_pids, find_profile_gateway_processes, find_windows_gateway_services
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    with _abort_on_error("Could not prepare Windows gateway pause for update"):
        ...
        from sage_cli.gateway import _capture_gateway_argv
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    PID.
    """
    from sage_cli.update_cmd import _desktop_owns_gateway_lifecycle, _m
    if not _m()._is_windows():
        return True
    with _abort_on_error("Could not load Windows gateway cold-start helpers"):
        from sage_cli import gateway_windows
        from sage_cli.gateway import find_gateway_pids
...
def _relaunch_paused_gateways(token: dict, profiles: dict, unmapped: list) -> tuple[list[str], int]:
    ...
    with _abort_on_error("Could not load Windows gateway restart helper"):
        from sage_cli.gateway import launch_detached_gateway_restart_by_cmdline, launch_detached_profile_gateway_restart
...
def _resume_windows_gateways_and_merge_outcome(outcome, _windows_gateway_resume, gateway_mode: bool):
    ...
    from sage_cli.update_cmd import _m, _write_gateway_update_exit_code
    try:
    ...
    with suppress(Exception):
        from sage_cli.update_receipt import record_gateway_restart
...
def _clear_windows_venv_holders_or_exit(args, gateway_mode: bool, _windows_gateway_resume):
    ...
    from sage_cli.update_cmd import _m, _record_update_step, _refuse_gateway_ancestor_tree_kill
