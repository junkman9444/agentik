...
dependency/config-schema warnings. Mixed into :class:`sage_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``sage_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from sage_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from sage_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from sage_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from sage_cli.plugins import LoadedPlugin

logger = logging.getLogger("sage_cli.plugins")
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        """
        from sage_cli.plugins import PluginContext, _PLUGINS_DEBUG
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from sage_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        from sage_cli.plugins_manifest import requires_hermes_error
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (sage_cli.plugin_compat).
        from sage_cli.plugin_compat import disable_reason
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from sage_cli.plugins import PluginContext
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from sage_cli.agent_plugins import load_agent_plugin
