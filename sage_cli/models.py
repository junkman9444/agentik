...
``sage_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from sage_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from sage_cli.models_reasoning_caps import (
    _OPENROUTER_CATALOG_URL,
    _seed_reasoning_caps)
from sage_cli.models_local import (
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from sage_cli.model_catalog import refresh_interval_seconds
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from sage_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from sage_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    ...
    sign-in completion in ``sage_cli.anon_auth`` so both land on the same model.
    """
...
def fetch_openrouter_models(
    ...
    try:
        from sage_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    try:
        from sage_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    from sage_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from sage_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from sage_cli.auth import get_auth_status, has_usable_secret
...
def _resolve_static_model_alias(
    ...
    try:
        from sage_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from sage_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``sage_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from sage_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from sage_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from sage_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from sage_cli.codex_models import get_codex_model_ids
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from sage_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from sage_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    if isinstance(request_headers, dict):
        ...
        from sage_cli.config import normalize_extra_headers
...
def cached_fetch_api_models(
    ...
    from sage_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('sage_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('sage_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('sage_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('sage_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('sage_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('sage_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('sage_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('sage_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('sage_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('sage_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('sage_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('sage_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('sage_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('sage_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('sage_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('sage_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('sage_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('sage_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('sage_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('sage_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('sage_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('sage_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('sage_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('sage_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('sage_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('sage_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('sage_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('sage_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('sage_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('sage_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('sage_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
