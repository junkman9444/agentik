...
from sage_cli.config import redact_key
from sage_cli.web_deps import late
from sage_cli.web_server_files import _path_is_under
from sage_cli.web_server_gateway import _restart_gateway_after
from sage_cli.web_server_memory import _normalize_memory_provider_name, _require_memory_provider_ready
...
from sage_cli.web_routers._common import _CONFIG_MUTATION_LOCK, http_failure, spawn_profile_action
from sage_cli.web_routers.files import stream_upload_to_path

_log = logging.getLogger("sage_cli.web_server")
router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_discover_memory_provider_statuses = late("_discover_memory_provider_statuses", "sage_cli.web_server_memory")
...
save_config = late("save_config", "sage_cli.config")
...
# --- Webhooks: same JSON store as the CLI (sage_cli.webhook); the adapter
    ...
    def _run():
        try:
            ...
            if not provider.startswith(CUSTOM_POOL_PREFIX):
                try:
                    from sage_cli.auth import _load_auth_store, unsuppress_credential_source
...
async def create_hook(body: HookCreate):
    ...
    valid_hooks = None
    with contextlib.suppress(Exception):
        from sage_cli.plugins import VALID_HOOKS as valid_hooks
