...
Extracted from ``sage_cli.web_server``; app state and helpers are late-bound through
:mod:`sage_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from sage_cli.web_deps import LateState, late
from sage_cli.web_server_gateway import _display_system_platform
...
from sage_cli.config import get_config_path, get_env_path
from sage_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from sage_cli.web_routers._common import scoped_to_thread
from pathlib import Path
from typing import Any, Dict, Optional

_log = logging.getLogger("sage_cli.web_server")
router = APIRouter()
# Mounted separately by web_server so /api/logs keeps its original route-table position.
logs_router = APIRouter()

# Late-bound so a test's monkeypatch on the owning module wins at call time.
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "sage_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "sage_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "sage_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "sage_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "sage_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "sage_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "sage_cli.web_server_lifecycle")
_spawn_hermes_action = late("_spawn_hermes_action", "sage_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "sage_cli.config")
get_hermes_home = late("get_hermes_home", "sage_cli.config")
...
load_config = late("load_config", "sage_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "sage_cli.web_server_sessions")
...
async def get_ssh_ownership(request: Request):
    from sage_cli.web_server import _SSH_OWNER_NONCE
...
# Profile segment mirrors sage_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
def _nous_session_validity() -> str:
    ...
    try:
        from sage_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from sage_cli.web_server import DASHBOARD_HEALTH
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from sage_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from sage_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of sage_cli.gateway blocks
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from sage_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from sage_cli.nous_compat import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    from sage_cli.debug import build_debug_share
...
async def get_logs(
    ...
    from sage_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from sage_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
