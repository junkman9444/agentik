...
from sage_cli.web_server_oauth import (
...
from sage_cli.web_models import OAuthSubmitBody
...
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "sage_cli.web_server_oauth")
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from sage_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from sage_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from sage_cli.auth import _save_codex_tokens
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        if status_fn is not None:
            return status_fn()
        from sage_cli import auth as hauth
...
async def _start_nous_device_code(profile: Optional[str]) -> Dict[str, Any]:
    ...
    from sage_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from sage_cli.web_server_profiles import _config_profile_scope, _profile_scope
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from sage_cli.auth import _xai_oauth_request_device_code
...
async def disconnect_oauth_provider(provider_id: str, request: Request, profile: Optional[str] = None):
    ...
    def _run():
        ...
        try:
            from sage_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
