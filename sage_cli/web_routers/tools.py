...
from sage_cli.web_server_profiles import _plugin_terminal_backend_rows
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from sage_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _toolset_model_catalog(ts_key: str, plugin_name: str):
    ...
    from sage_cli.tools_config import _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    ...
    from sage_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
def _find_toolset_provider_row(
    ...
    from sage_cli.tools_config import _is_provider_active
...
async def get_toolsets(profile: Optional[str] = None):
    from sage_cli.tools_config import (
        ...
        gui_toolset_label)
    from sage_cli.platforms import platform_label
...
async def select_toolset_provider(
    ...
    """
    from sage_cli.tools_config import apply_provider_selection, web_provider_capabilities
...
async def run_toolset_post_setup(
    ...
    from sage_cli.tools_config import valid_post_setup_keys
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('sage_cli.web_deps', 'LateState'),
}
