...
Shared helpers are reached via the late-binding seam in :mod:`sage_cli.web_deps`
...
import copy
import functools
from sage_cli.web_read_coalescing import coalesced_read
import inspect
import json
import logging
import re
...
from sage_cli.web_server_config import _apply_main_model_assignment, _normalize_main_model_assignment
from sage_cli.web_server_gateway import _strip_session_list_rows
from sage_cli.web_server_profiles import (
    _fallback_profile_dicts, _hub_action_name, _write_profile_mcp_servers,
)
from sage_cli.web_server_sessions import _open_session_db_at_path
...
from sage_cli.web_models import (
    ...
    SessionPrScanBody)
from sage_cli.web_server_profiles import _hermes_home_scope
...
_cron_profile_home = late("_cron_profile_home", "sage_cli.web_server_cron")
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from sage_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map sage_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
# ── Export / Import ── wraps sage_cli.profiles.export_profile / import_profile. Paths are
