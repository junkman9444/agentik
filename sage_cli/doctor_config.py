...
Split out of ``sage_cli/doctor.py``."""
...
def _has_provider_env_config(content: str) -> bool:
    ...
    from sage_cli.doctor import _PROVIDER_ENV_HINTS
...
def collect_relay_plugin_cutover_findings(raw_config: dict | None, env_map: dict | None) -> list[tuple[str, str]]:
    ...
    from sage_cli.relay_plugin_cutover import (LEGACY_RELAY_EXPORT_ENV_VARS, RELAY_PLUGINS_CONFIG_ENV,
...
def _check_env_file(should_fix: bool, f: Finding) -> None:
    ...
    from sage_cli.doctor import HERMES_HOME, PROJECT_ROOT, _DHH
    managed_scope_check()
...
def _known_provider_ids(cfg: dict) -> tuple[set, list, object, object, object]:
    ...
    with warn_on_error(""):
        from sage_cli.auth import PROVIDER_REGISTRY, resolve_provider as resolve_auth
        known = set(PROVIDER_REGISTRY.keys()) | {"openrouter", "custom", "auto", "moa"}
    with warn_on_error(""):
        from sage_cli.config import get_compatible_custom_providers
        from sage_cli.providers import custom_provider_aliases as aliases, normalize_provider as normalize, resolve_provider_full as resolve_full
        with warn_on_error(""):
...
def _validate_model_config(config_path, issues: list) -> None:
    ...
    if runtime_provider and runtime_provider not in ("auto", "custom"):
        from sage_cli.doctor import _DHH
        with warn_on_error(""):
...
def _check_config_file(should_fix: bool, f: Finding) -> None:
    ...
    if config_path.exists():
        ...
        if example_config.exists():
            ...
            from sage_cli.config import DEFAULT_CONFIG, save_config
...
def _drift_max_iterations_ghost(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from sage_cli.config import load_env, read_user_config_raw, remove_env_value
    raw_config = read_user_config_raw(config_path)
...
def _drift_deprecations(f: Finding, should_fix: bool, config_path) -> None:
    ...
    from sage_cli.config import load_env, read_user_config_raw
...
def _check_xai_retirement(should_fix: bool, f: Finding) -> None:
    from sage_cli.config import load_config
    from sage_cli.xai_retirement import MIGRATION_GUIDE_URL, find_retired_xai_refs, format_issue
...
def _check_plugin_compat(should_fix: bool, f: Finding) -> None:
    from sage_cli.plugin_compat import ALLOW_KEY, COMPAT_REMOVAL, compat_report, removal_in_effect
