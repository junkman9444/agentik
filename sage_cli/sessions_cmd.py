...
``sage_cli.main`` at call time so monkeypatches keep working. Picker: :mod:`sage_cli.sessions_cmd_browse`.
"""
...
from sage_cli.sessions_cmd_browse import _relative_time, _session_browse_picker
...
def _print_dry_run_preview(candidates, filters) -> None:
    from sage_cli.session_filters import describe_filters
...
def _cmd_repair(args):
    from sage_state import DEFAULT_DB_PATH as db_path, SessionDB
...
def _cmd_import(args):
    from sage_cli.foreign_sessions import run_sessions_import
...
def _cmd_list(db, args):
    from sage_state_sessions import workspace_key as _ws_key
...
def _cmd_export(db, args):
    from sage_cli.session_filters import build_prune_filters
    filters = None
    ...
    def _redact(data):
        if not args.redact or data is None:
            return data
        from sage_cli.session_export_md import redact_session_data
...
def _render_only(args, sessions):
    ...
    from sage_cli.session_export import export_record_count, render_sessions_export
...
def _render_html(args, sessions):
    ...
    from sage_cli.session_export_html import generate_html_export, generate_multi_session_html_export
...
def _export_markdown(db, args, filters, redact):
    ...
    from sage_cli.session_export_md import append_manifest_entry, write_session_markdown
    if args.output == "-":
...
def _export_markdown_single(db, args, export_one, output_dir, lineage_is_logical):
    ...
    from sage_cli.session_export_md import verify_export_file
...
def _prune_never_active_keyed(db, args):
    ...
    """
    from sage_cli.session_filters import format_epoch, parse_duration_seconds
...
def _cmd_prune_or_archive(db, args, action):
    ...
    from sage_cli.session_filters import build_prune_filters, describe_filters, format_epoch
