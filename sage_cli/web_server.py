...
Usage: ``python -m sage_cli.main web [--port 8080]``.
"""
...
from sage_cli.install_identity import get_install_id as _shared_get_install_id
from sage_cli.pty_session import run_reaper
...
from sage_cli.web_server_lifecycle import (  # noqa: E402
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            ...
            if len(profile_homes) > 1:
                ...
                from sage_cli.profiles import _check_gateway_running
...
async def _lifespan(app: "FastAPI"):
    ...
    # Import sage_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    if os.getenv("HERMES_DESKTOP") == "1":
        ...
        try:
            from sage_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from sage_cli.config import load_config
            from sage_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    from sage_cli.free_tier_bootstrap import start_background_bootstrap
    ...
    try:
        ...
        try:
            from sage_cli.local_runtime.bootstrap import shutdown_local_runtime
...
from sage_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
from sage_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from sage_cli.dashboard_auth.prefix import resolve_public_url
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from sage_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from sage_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from sage_cli.dashboard_auth.token_auth import token_auth_middleware
...
from sage_cli import web_server_gateway as _gateway_mod  # noqa: E402
from sage_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from sage_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from sage_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from sage_cli.web_server_dashboard import (  # noqa: E402
...
from sage_cli.web_routers import (  # noqa: E402
...
from sage_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    try:
        ...
        from sage_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
...
def _configure_auth_gate(
    ...
    if app.state.auth_required:
        ...
        from sage_cli.dashboard_auth import list_providers
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from sage_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from sage_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from sage_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from sage_cli.mcp_startup import defer_background_mcp_discovery
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('sage_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('sage_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('sage_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('sage_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('sage_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('sage_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('sage_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('sage_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('sage_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('sage_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('sage_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('sage_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('sage_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('sage_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('sage_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('sage_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('sage_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('sage_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('sage_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('sage_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('sage_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('sage_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('sage_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('sage_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('sage_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('sage_cli.web_models', 'HookCreate'),
    'HookDelete': ('sage_cli.web_models', 'HookDelete'),
    'ImportRequest': ('sage_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('sage_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('sage_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('sage_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('sage_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('sage_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('sage_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('sage_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('sage_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('sage_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('sage_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('sage_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('sage_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('sage_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('sage_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('sage_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('sage_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('sage_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('sage_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('sage_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('sage_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('sage_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('sage_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('sage_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('sage_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('sage_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('sage_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('sage_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('sage_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('sage_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('sage_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('sage_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('sage_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('sage_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('sage_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('sage_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('sage_cli.web_models', 'SessionImport'),
    'SessionPrune': ('sage_cli.web_models', 'SessionPrune'),
    'SessionRename': ('sage_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('sage_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('sage_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('sage_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('sage_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('sage_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('sage_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('sage_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('sage_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('sage_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('sage_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('sage_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('sage_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('sage_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('sage_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('sage_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('sage_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('sage_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('sage_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('sage_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('sage_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('sage_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('sage_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('sage_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('sage_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('sage_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('sage_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('sage_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('sage_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('sage_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('sage_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('sage_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('sage_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('sage_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('sage_cli.config', 'cfg_get'),
    'check_config_version': ('sage_cli.config', 'check_config_version'),
    'check_hermes_update': ('sage_cli.web_routers.actions', 'check_hermes_update'),
    'clear_model_endpoint_credentials': ('sage_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('sage_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('sage_cli.config', 'coerce_provider_id'),
    'console_ws': ('sage_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('sage_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('sage_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('sage_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('sage_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('sage_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('sage_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('sage_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('sage_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('sage_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('sage_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('sage_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('sage_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('sage_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('sage_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('sage_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('sage_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('sage_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('sage_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('sage_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('sage_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('sage_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('sage_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('sage_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('sage_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('sage_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('sage_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('sage_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('sage_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('sage_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('sage_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('sage_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('sage_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('sage_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('sage_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('sage_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('sage_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('sage_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('sage_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('sage_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('sage_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('sage_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('sage_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('sage_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('sage_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('sage_cli.config', 'get_config_path'),
    'get_config_raw': ('sage_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('sage_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('sage_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('sage_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('sage_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('sage_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('sage_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('sage_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('sage_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('sage_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('sage_cli.config', 'get_env_path'),
    'get_env_vars': ('sage_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('sage_cli.web_routers.status', 'get_health'),
    'get_hermes_home': ('sage_cli.config', 'get_hermes_home'),
    'get_learning_graph': ('sage_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('sage_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('sage_cli.web_routers.status', 'get_logs'),
    'get_media': ('sage_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('sage_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('sage_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('sage_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('sage_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('sage_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('sage_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('sage_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('sage_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('sage_cli.web_routers.status', 'get_portal_status'),
    'get_process_hermes_home': ('sage_cli.config', 'get_process_hermes_home'),
    'get_profile_setup_command': ('sage_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('sage_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('sage_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('sage_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('sage_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('sage_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('sage_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('sage_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('sage_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('sage_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('sage_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('sage_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('sage_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('sage_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('sage_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('sage_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('sage_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('sage_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('sage_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('sage_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('sage_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('sage_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('sage_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('sage_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('sage_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('sage_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('sage_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('sage_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('sage_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('sage_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('sage_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('sage_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('sage_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('sage_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('sage_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('sage_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('sage_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('sage_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('sage_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('sage_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('sage_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('sage_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('sage_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('sage_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('sage_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('sage_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('sage_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('sage_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('sage_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('sage_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('sage_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('sage_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('sage_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('sage_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('sage_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('sage_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('sage_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('sage_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('sage_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('sage_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('sage_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('sage_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('sage_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('sage_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('sage_cli.config', 'load_env'),
    'mcp_oauth_callback': ('sage_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('sage_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('sage_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('sage_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('sage_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('sage_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('sage_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('sage_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('sage_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('sage_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('sage_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('sage_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('sage_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('sage_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('sage_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('sage_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('sage_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('sage_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('sage_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('sage_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('sage_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('sage_cli.config', 'remove_env_value'),
    'remove_env_var': ('sage_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('sage_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('sage_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('sage_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('sage_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('sage_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('sage_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('sage_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('sage_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('sage_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('sage_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('sage_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('sage_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('sage_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('sage_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('sage_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('sage_cli.doctor', 'run_doctor'),
    'run_dump': ('sage_cli.dump', 'run_dump'),
    'run_import': ('sage_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('sage_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('sage_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('sage_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('sage_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('sage_cli.config', 'save_config'),
    'save_env_value': ('sage_cli.config', 'save_env_value'),
    'save_toolset_env': ('sage_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('sage_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('sage_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('sage_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('sage_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('sage_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('sage_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('sage_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('sage_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('sage_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('sage_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('sage_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('sage_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('sage_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('sage_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('sage_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('sage_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('sage_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('sage_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('sage_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('sage_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('sage_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('sage_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('sage_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('sage_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('sage_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('sage_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('sage_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('sage_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('sage_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('sage_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('sage_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('sage_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('sage_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('sage_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('sage_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('sage_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('sage_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('sage_cli.web_routers.cron', 'update_cron_job'),
    'update_hermes': ('sage_cli.web_routers.actions', 'update_hermes'),
    'update_learning_node': ('sage_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('sage_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('sage_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('sage_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('sage_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('sage_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('sage_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('sage_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('sage_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('sage_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('sage_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('sage_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('sage_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('sage_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('sage_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('sage_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('sage_cli.config', 'write_platform_config_field'),
}
