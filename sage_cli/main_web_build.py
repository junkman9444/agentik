...
logger = logging.getLogger("sage_cli.main")
...
def _record_bytecode_fingerprint() -> None:
    ...
    from sage_cli.main import PROJECT_ROOT, _read_git_revision_fingerprint
    try:
...
def _sweep_stale_bytecode_if_checkout_changed() -> None:
    ...
    """
    from sage_cli.main import PROJECT_ROOT, _clear_bytecode_cache, _read_git_revision_fingerprint
    try:
        fingerprint = _read_git_revision_fingerprint(PROJECT_ROOT)
        if not fingerprint:
...
def _web_dist_dir(web_dir: Path) -> Path:
    """Vite outputs to ``sage_cli/web_dist/`` (vite.config.ts outDir), NOT ``web/dist/``."""
    return _web_project_root(web_dir) / "sage_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    half-state with the ``hermes`` launcher present but ``sage_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    from sage_cli.npm_engine import maybe_repair_npm_engine
...
def _web_npm_install_context(web_dir: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    """
    from sage_cli.main import _is_termux_startup_environment
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    """
    from sage_cli.main_install_repair import _resolve_node_runtime_npm
