...
cycle and lets tests patch ``sage_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`sage_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from sage_cli.config import clear_model_endpoint_credentials
...
from sage_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from sage_cli.model_setup_flows_azure import _model_flow_azure_foundry
from sage_cli.model_setup_flows_bedrock import _model_flow_bedrock
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from sage_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from sage_cli.models import model_ids
    from sage_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from sage_cli.main_provider_setup import _prompt_api_key
    from sage_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from sage_cli.config import get_env_value
    ...
    from sage_cli.models import ai_gateway_model_ids
    from sage_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from sage_cli.auth import _save_model_choice
    from sage_cli.moa_config import normalize_moa_config
...
def _nous_model_catalog(free_tier: bool, portal_url: str, model_ids: list, pricing: dict):
    ...
    if free_tier:
        try:
            from sage_cli.nous_compat import format_nous_portal_entitlement_message, get_nous_portal_account_info
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from sage_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _nous_persist_selection(selected: str, creds: dict) -> dict:
    ...
    from sage_cli.auth import _save_model_choice, _update_config_for_provider
    from sage_cli.config import get_env_value, load_config, save_config, save_env_value
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from sage_cli.auth import get_provider_auth_state, _prompt_model_selection, _login_nous, PROVIDER_REGISTRY
    from sage_cli.config import load_config
    from sage_cli.nous_compat import prompt_enable_tool_gateway
    ...
    from sage_cli.models import check_nous_free_tier, get_curated_nous_model_ids
    from sage_cli.models_pricing import get_pricing_for_provider
    from sage_cli.model_switch_providers import _free_tier_nous_row
    ...
    from sage_cli.nous_compat import nous_policy_notice
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from sage_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from sage_cli.auth import (
        get_qwen_auth_status, resolve_qwen_runtime_credentials, _prompt_model_selection, DEFAULT_QWEN_BASE_URL)
    from sage_cli.models import fetch_api_models
...
def _copilot_catalog(api_key: str):
    ...
    from sage_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from sage_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from sage_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from sage_cli.main_provider_setup import _prompt_reasoning_effort_selection
    from sage_cli.setup import _current_reasoning_effort, _set_reasoning_effort
    from sage_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from sage_cli.config import load_config
    from sage_cli.models import fetch_api_models, github_model_reasoning_efforts, copilot_model_api_mode
    provider_id = "copilot"
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from sage_cli.env_loader import format_secret_source_suffix
...
def _model_flow_kimi(config, current_model=""):
    ...
    from sage_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from sage_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from sage_cli.auth import PROVIDER_REGISTRY
    from sage_cli.config import save_env_value
    from sage_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    from sage_cli.auth import _prompt_model_selection
    from sage_cli.config import load_config, get_env_value
    from sage_cli.models import _PROVIDER_MODELS
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``sage_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from sage_cli.main_provider_setup import _prompt_provider_choice
    from sage_cli.auth import ZAI_ENDPOINTS
...
def _lmstudio_models(pconfig, curated, api_key, base_url):
    ...
    from sage_cli.models_local import fetch_lmstudio_models
    try:
...
def _ollama_cloud_models(pconfig, curated, api_key, base_url):
    ...
    from sage_cli.models import fetch_ollama_cloud_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from sage_cli.auth import PROVIDER_REGISTRY
    from sage_cli.config import save_env_value, load_config
    from sage_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
    ...
    if provider_id == "actual":
        from sage_cli.providers import normalize_provider
...
def _anthropic_authenticate() -> bool:
    ...
    from sage_cli.main_provider_setup import _run_anthropic_oauth_flow
    from sage_cli.config import save_env_value, save_anthropic_api_key
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from sage_cli.auth import get_anthropic_key
    from sage_cli.models import _PROVIDER_MODELS
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('sage_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('sage_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('sage_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('sage_cli.providers', 'custom_provider_slug'),
