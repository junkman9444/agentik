...
per function so test patches on ``sage_cli.setup`` take effect."""
...
def _blank_slate_done(config: dict, hermes_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    ...
    from sage_cli.setup import _info, _print_setup_summary, print_success
    print()
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from sage_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    from sage_cli.setup import _info
    try:
        from sage_cli.model_setup_flows import _model_flow_nous
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from sage_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    from sage_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
