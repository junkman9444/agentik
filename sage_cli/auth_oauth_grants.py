...
Split out of ``sage_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``sage_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from sage_cli.auth_constants import _decode_jwt_claims
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    """
    from sage_cli.auth import _same_path, _save_auth_store
...
def _oauth_heal_clean_mark_path() -> Optional[Path]:
    ...
    try:
        from sage_cli.auth import _auth_file_path
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    """
    from sage_cli.auth import _oauth_heal_notices
...
class _HealPass:
    ...
    def heal_profile_singleton(self, profile_singleton: Optional[Path]) -> None:
        if profile_singleton is None or not profile_singleton.exists():
            return
        from sage_cli.auth import _is_same_auth_store
