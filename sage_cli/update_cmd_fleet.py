...
Split out of ``sage_cli/update_cmd.py``; every name is re-imported there so
``sage_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from sage_cli.update_inventory import _gateway_service_matches_profile
...
def _current_checkout_sha() -> str | None:
    ...
    from sage_cli.update_cmd import _capture_head_sha, _m
    try:
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from sage_cli.update_cmd import _current_checkout_sha
    try:
        from sage_cli.update_receipt import read_latest_receipt
...
def _live_fleet_covers_receipt(expected_sha: str | None) -> bool:
    ...
    if not expected_sha:
        return False
    from sage_cli.update_receipt import collect_fleet_versions, read_latest_receipt
...
def _apply_pending_fleet_restart_catchup() -> None:
    ...
    """
    from sage_cli.update_cmd import _run_pending_fleet_restart
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from sage_cli.gateway import is_macos
    if not failed_units:
        return
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``sage_cli.gateway`` no longer importing
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``sage_cli.gateway`` in the
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``sage_cli.gateway`` from the freshly
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    ...
    """
    from sage_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from sage_cli.gateway import find_gateway_pids, _get_service_pids
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    try:
        ...
        from sage_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from sage_cli.gateway import _get_restart_exit_wait_budget
...
def _print_legacy_units_warning() -> None:
    ...
    from sage_cli.gateway import (has_legacy_hermes_units, _find_legacy_hermes_units, supports_systemd_services)
...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    """
    from sage_cli.update_receipt import collect_fleet_versions
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from sage_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from sage_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import sage_cli.update_receipt as _ur
    ...
    with _best_effort('Update receipt finalize failed: %s'):
        from sage_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``sage_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
