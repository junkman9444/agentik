...
Split out of ``sage_cli.models``. Catalog fetchers defined in ``sage_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("sage_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``sage_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from sage_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from sage_cli import models as _m
    from sage_cli import models_local as _ml
    from sage_cli.models_local import _configured_ollama_base_url, _drop_authorization
