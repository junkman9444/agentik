...
in :func:`run_migrations`. Every write goes through ``sage_cli.config._persist_migration`` so a
...
def _cfg():
    """Return the live ``sage_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from sage_cli.relay_plugin_cutover import legacy_relay_plugin_keys
