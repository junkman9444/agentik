...
Names are re-imported by ``update_cmd`` (``sage_cli.update_cmd.<name>`` resolves/monkeypatches);
...
def _reload_config_modules() -> None:
    ...
    for mod_name in (
        "sage_cli.config_defaults", "sage_cli.config", "sage_cli.config_migrations",
...
def _run_config_check_fresh() -> tuple:
    ...
    from sage_cli.update_cmd import _reload_config_modules
    _reload_config_modules()
    from sage_cli.config import check_config_version
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    """
    from sage_cli.update_cmd import _run_config_check_fresh, _run_migrate_config_fresh
    ...
    with _best_effort('Sibling profile enumeration failed: %s'):
        ...
        from sage_cli.profiles import _get_profiles_root, _PROFILE_ID_RE
...
def _restore_snapshot_safety_nets(pre_update_snapshot_id) -> None:
    ...
    *pre_update_snapshot_id*) and every sibling profile (against ITS OWN snapshot)."""
    ...
    with _best_effort('Sibling cron auto-restore check failed: %s'):
        from sage_cli.backup import restore_cron_jobs_all_profiles
    ...
    with _best_effort('Sibling config auto-restore check failed: %s'):
        from sage_cli.backup import restore_config_model_settings_all_profiles
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from sage_cli.update_cmd import _gateway_prompt
    if assume_yes:
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    _reload_config_modules()
    from sage_cli.config import get_missing_env_vars, get_missing_config_fields
