...
# parse-time from ``sage_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
from sage_cli._secrets_common import (
...
from sage_cli.config import get_env_path, load_config, save_config, save_env_value
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``sage_cli.secrets_cli.bw`` directly.
    ...
    so ``import sage_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
# ── Argparse wiring — called from sage_cli.main ──
