...
Imports from ``sage_cli.profiles`` stay lazy (inside each handler) so tests can monkeypatch
...
def _render_distribution_plan(plan) -> None:
    ...
    from sage_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def _profile_status(args):
    ...
    from sage_cli.profiles import format_profile_label, get_active_profile_name, list_profiles
...
def _profile_use(args):
    from sage_cli.profiles import set_active_profile
...
def _profile_delete(args):
    from sage_cli.profiles import delete_profile
    try:
...
def _describe_target_dir(name: str) -> Path:
    ...
    from sage_cli import profiles as _profiles_mod
    if _profiles_mod.normalize_profile_name(name) == "default":
        from sage_constants import get_hermes_home as _hh
        return Path(_hh())
...
def _profile_describe(args):
    ...
    from sage_cli import profile_describer as _pd
    if all_flag:
...
def _profile_rename(args):
    from sage_cli.profiles import normalize_profile_name, rename_profile
    try:
...
def _profile_export(args):
    from sage_cli.profiles import export_profile, get_profile_export_path
    name = args.profile_name
    try:
...
def _profile_install(args):
    import tempfile
    from sage_cli.profile_distribution import DistributionError, install_distribution, plan_install
    try:
...
def _profile_update(args):
    from sage_cli.profile_distribution import DistributionError, read_manifest, update_distribution
    from sage_cli.profiles import get_profile_dir, normalize_profile_name
    try:
...
def _profile_info(args):
    from sage_cli.profile_distribution import describe_distribution, DistributionError
    try:
