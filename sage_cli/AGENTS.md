# sage_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles
...
`sage_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
Rules: all interactive menu-pickers use curses (`sage_cli/curses_ui.py`; example
`sage_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`sage_cli/commands.py`)
...
`sage_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`sage_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`hermes tools/setup`, most subcommands, `sage_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`sage_cli/skin_engine.py`)
...
`_apply_profile_override()` in `sage_cli/main.py` sets `HERMES_HOME` before any module import, so
...
## Nous free tier (`sage_cli/anon_auth.py`)
