...
``sage_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
_UPDATE_CRITICAL_MODULES = "sage_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from sage_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from sage_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    ...
    probe = (
        ...
        # only our own packages count. Roots come from sage_constants so the user hint can't drift.
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    try:
        from sage_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    from sage_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> list[str]:
    ...
    from sage_constants import with_hermes_node_path
...
# canonical case: ``sage_cli.main`` used to import it at startup while resolving external secret sources;
...
def _rebuild_desktop_after_update(
    ...
    desktop_build_cmd = [sys.executable, "-m", "sage_cli.main", "desktop", "--build-only"]
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    """
    from sage_cli.update_cmd import _path_uid
    try:
