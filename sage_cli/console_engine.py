...
def _split_line(line: str) -> list[str]:
    ...
    # See #78293.
    # See #83934.
    from sage_cli._subprocess_compat import split_command_line
    try:
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from sage_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from sage_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        main_handler = (
            (lambda: getattr(importlib.import_module("sage_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"sage_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"sage_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "project": (
        _CliSurface("builder", "sage_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "sage_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "sage_cli.send_cmd", "register_send_subparser")
...
_version = _simple_command(
    "version", "sage_cli._startup_fast", "print_fast_version_info", check_updates=True)


def _status(_engine: HermesConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "status")
    from sage_cli.status import show_status
...
_doctor = _simple_command(
    "doctor", "sage_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "sage_cli.config", "show_config")
_cron_status = _simple_command("cron status", "sage_cli.cron", "cron_status")
...
def _logs(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from sage_cli.logs import list_logs, tail_log
    if ns.log_name == "list":
...
def _config_migrate(_engine: HermesConsoleEngine, args: list[str]) -> None:
    ...
    from sage_cli.config import migrate_config
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from sage_state import SessionExportTooLargeError, resolved_max_export_messages
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> None:
    ...
    from sage_state import DEFAULT_DB_PATH
    from sage_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _cron_list(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    from sage_cli.cron import cron_list
