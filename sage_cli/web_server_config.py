...
from sage_cli.web_server_memory import _normalize_memory_provider_name
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    from sage_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
    from sage_cli.model_normalize import normalize_model_for_provider
    from sage_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and not is_custom_provider_slug and "/" in model_in:
        ...
        if cur_provider and normalize_provider(cur_provider) in _AGGREGATOR_PROVIDERS:
            ...
            from sage_cli.models_detect import provider_has_credentials
...
# in sage_cli/config.py — listed here for deterministic ordering in the UI.
...
def _apply_nous_gateway_defaults(cfg: dict) -> list:
    ...
    *unconfigured* tools through the Nous Tool Gateway. Purely additive — tools with a direct
    ...
    try:
        from sage_cli.nous_compat import apply_nous_managed_defaults
...
def _register_custom_endpoint(base_url: str, api_key: str, model: str) -> None:
    ...
    try:
        from sage_cli.main_provider_setup import _auto_provider_name, _save_custom_provider
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from sage_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
