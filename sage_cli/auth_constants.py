...
Pure leaf: imports nothing from ``sage_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because sage_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("sage_cli.auth.httpx.Client")
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from sage_cli import __version__ as _HERMES_CLI_VERSION
