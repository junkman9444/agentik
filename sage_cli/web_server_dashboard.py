...
from sage_cli.config import cfg_get, get_process_hermes_home
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    ``sage_cli.dashboard_auth.prefix`` so gate, OAuth, cookies and SPA mount agree)."""
    from sage_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    """
    from sage_cli.web_server import WEB_DIST, _DASHBOARD_EMBEDDED_CHAT_ENABLED, app
    from sage_cli.web_deps import _server
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    ~/.sage/, the same trust level as config.yaml. Empty asset values are dropped so a
...
def _dashboard_plugin_search_dirs() -> List[tuple]:
    ...
    """
    from sage_cli.plugins import get_bundled_plugins_dir
    ...
    # the hermes root (``~/.sage/plugins``). Scan the default root as well (``get_default_hermes_root()``
    # unwraps ``<root>/profiles/<name>`` → ``<root>`` and returns a custom ``HERMES_HOME`` unchanged when it
    # *is* the root), mirroring how ``sage_cli.plugins`` resolves plugin install locations. The
    ...
    # ``true`` / ``yes`` / ``on``) so the gate matches ``sage_cli/plugins.py`` and the documented user
    # contract.
    if env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".sage" / "plugins", "project"))
    return search_dirs
...
def _discover_dashboard_plugins() -> list:
    ...
    project plugin sources — same three sources as ``sage_cli.plugins``."""
    plugins = []
    seen_names: set = set()
...
def _merged_plugins_hub(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    """
    from sage_cli.web_server_memory import _discover_memory_provider_statuses, _normalize_memory_provider_name
    from sage_cli.web_server import _get_dashboard_plugins
    from sage_cli.config import get_hermes_home, load_config
    global _plugins_hub_cache, _plugins_hub_cache_expires_at
    now = time.monotonic()
    ...
    from sage_cli.plugins_cmd_catalog import removed_annotation
...
def _mount_plugin_api_routes():
    ...
    """
    from sage_cli.web_server import _get_dashboard_plugins, app
    try:
