...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    guard keeps this path consistent with ``sage_state_repair._backup_db_file``.
    ...
    ``sage_cli.sqlite_safe_read`` exists to prevent (see #71724). Holding the lock means no connection can
...
def _lost_and_found_plausibility_errors(
    ...
    """
    from sage_cli.session_lost_and_found import _EPOCH_LOW, STUB_TITLE_PREFIX
