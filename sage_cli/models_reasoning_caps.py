...
Split out of ``sage_cli.models``. OpenRouter and
...
``sage_cli.models`` — tests reset them there — and are read/written by attribute name.
...
class _CapsSource:
    """One catalog's cache slots on ``sage_cli.models`` plus how to name its URL.
