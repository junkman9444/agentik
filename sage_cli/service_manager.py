...
def detect_service_manager() -> ServiceManagerKind:
    ...
    from sage_cli.gateway import is_macos, is_windows, supports_systemd_services
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``sage_cli.gateway`` (systemd/launchd) and
# ``sage_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
...
class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``sage_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import sage_cli
        importlib.import_module(f"sage_cli.{self._backend}")
        return getattr(sage_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in sage_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in sage_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``sage_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        deterministic SHA-256 allocator in ``sage_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        # sage_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
