...
# Re-exported: callers/tests patch sage_cli.model_switch.<name>.
from sage_cli.model_switch_providers import list_authenticated_providers
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from sage_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    if current_provider:
        try:
            from sage_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
...
def _ollama_configured_base() -> tuple[dict, str]:
    from sage_cli.models import _get_provider_config_dict
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from sage_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from sage_cli.config import load_config
        from sage_cli.moa_config import exact_moa_preset_name, normalize_moa_config
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from sage_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from sage_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from sage_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from sage_cli.models import opencode_model_api_mode
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from sage_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    try:
        from sage_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('sage_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('sage_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
