...
from sage_cli.middleware import VALID_MIDDLEWARE
from sage_cli.plugin_capabilities import plugin_capability_granted
from sage_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from sage_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from sage_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from sage_cli.plugins_loader import (
...
from sage_cli.plugins_dispatch import (  # noqa: F401 — re-exported
...
from sage_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from sage_cli.plugins_state import (
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (sage_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: sage_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
...
class PluginContext:
    ...
    def platform_actions(self):
        ...
        from sage_cli.platform_actions import PlatformActions
    ...
    def register_approval_transport(self, name: str, present_fn: Callable) -> None:
        ...
        from sage_cli.approval_transport import RegisteredApprovalTransport
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`sage_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
        auth gate (non-loopback bind without ``--insecure``). Wrong type / duplicate name warn and
        are ignored, never raised."""
        from sage_cli.dashboard_auth import DashboardAuthProvider
        from sage_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
    ...
    def register_auxiliary_task(
        ...
        from sage_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh HERMES_HOME/.plugin-compat-report.json from this discovery pass (sage_cli.plugin_compat).
        ...
        """
        try:
            from sage_cli.plugin_compat import compat_report
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                from sage_cli.agent_plugins import _discover_mcp
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from sage_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def _get_pre_tool_call_directive_details(
    ...
    from sage_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('sage_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('sage_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('sage_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('sage_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('sage_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('sage_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('sage_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('sage_cli.plugins_dispatch', 'format_system_prompt_section'),
