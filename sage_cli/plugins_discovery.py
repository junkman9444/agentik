...
Split out of :mod:`sage_cli.plugins`. Names that tests patch on the origin
...
from sage_cli.plugin_capabilities import VALID_CAPABILITY_IDS
from sage_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
from sage_cli.plugins_manifest import (
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from sage_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    if _origin._env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".sage" / "plugins"
