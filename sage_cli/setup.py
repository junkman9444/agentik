...
``sage_cli.setup.<name>`` keep working.
"""
...
from sage_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
...
from sage_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Sage Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def _setup_navigation_scope():
    ...
    from sage_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``HERMES_NONINTERACTIVE=1`` (``sage_cli/web_server.py``), where a
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from sage_cli.curses_ui import curses_checklist
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from sage_cli.main import select_provider_and_model
    try:
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    from sage_cli.tools_config import tools_command
...
# sage_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from sage_cli.setup_tts import setup_tts  # noqa: E402
from sage_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from sage_cli.setup_platforms import setup_gateway  # noqa: E402
from sage_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from sage_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from sage_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     ⚕ Sage Setup — {label:<36s} │")
...
def _run_full_setup(config: dict, hermes_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    def _gateway_step() -> None:
        ...
        from sage_cli.gateway import ensure_gateway_service
...
_PLUGIN_COMPAT_LAZY = {
    'get_nous_subscription_features': ('sage_cli.nous_compat', 'get_nous_subscription_features'),
    'get_optional_skills_dir': ('sage_constants', 'get_optional_skills_dir'),
