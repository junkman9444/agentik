...
helpers on sage_cli.setup, so those are imported lazily per function."""
...
def _setup_telegram_auto_result():
    ...
    from sage_cli.setup import get_hermes_home
    try:
        from sage_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _declines_reconfigure(env_var: str, label: str, question: str) -> bool:
    ...
    from sage_cli.setup import get_env_value, print_info, prompt_yes_no
...
def _save_prompted(env_var: str, question: str, *, password: bool = False, success_msg: str | None = None,
    ...
    """
    from sage_cli.setup import print_success, print_warning, prompt, save_env_value
...
def _save_allowlist(env_var: str, users: str, success_msg: str) -> None:
    ...
    from sage_cli.setup import print_success, save_env_value
...
def _prompt_allowlist(env_var: str, question: str, success_msg: str, open_msg: str, preset: str | None = None) -> str:
    ...
    from sage_cli.setup import print_info, prompt
...
def _save_port(env_var: str, value: str, default: str) -> None:
    ...
    from sage_cli.setup import print_success, print_warning, save_env_value
    if not value:
        return
    try:
...
def _prompt_telegram_bot_token() -> str | None:
    from sage_cli.setup import print_error, print_info, prompt
...
def _telegram_allowlist_nudge() -> None:
    ...
    from sage_cli.setup import get_env_value, print_info, prompt, prompt_yes_no
...
def _obtain_telegram_token():
    ...
    from sage_cli.setup import _info, print_error, prompt
...
def _setup_telegram():
    ...
    from sage_cli.setup import _info, print_info, print_header, print_success, prompt, prompt_yes_no, save_env_value
...
def _setup_bluebubbles():
    ...
    from sage_cli.setup import _info, print_header, print_success, prompt, prompt_yes_no
...
def _setup_webhooks():
    ...
    from sage_cli.setup import _info, print_header, print_success, print_warning, prompt, save_env_value
...
def _warn_missing_home_channels() -> None:
    ...
    from sage_cli.setup import get_env_value, _info, print_warning
    missing_home = [
...
def _restart_running_gateway(any_messaging: bool, supports_systemd: bool) -> None:
    ...
    from sage_cli.setup import print_error, prompt_yes_no
