...
from sage_cli.model_setup_flows_common import _say
...
def cmd_whatsapp(args):
    ...
    from sage_cli.main import _require_tty, get_hermes_home
    ...
    if wa_mode == "bot":
        _say("  Next steps:", "    1. Start the gateway:  hermes gateway",
             ...
             "  Tip: Agent responses are prefixed with '⚕ Sage'")
    else:
        _say("  Next steps:", "    1. Start the gateway:  hermes gateway",
...
def cmd_whatsapp_cloud(args):
    ...
    ``hermes whatsapp`` Baileys bridge wizard. See ``sage_cli/setup_whatsapp_cloud.py``."""
    from sage_cli.main import _require_tty
    _require_tty("whatsapp-cloud")
    from sage_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from sage_cli.slack_cli import slack_manifest_command
