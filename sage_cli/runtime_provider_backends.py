"""Provider-specific runtime builders for :mod:`sage_cli.runtime_provider`: Azure Foundry, the
...
``sage_cli.runtime_provider.*`` (``_get_model_config``, ``load_config``, ``has_usable_secret``,
...
def _resolve_bedrock_runtime(requested_provider: str, model_cfg: Dict[str, Any], target_model: Optional[str]) -> Dict[str, Any]:
    ...
    from sage_cli.config import load_config  # direct (not the origin delegate), as before
    rp = _rp()
