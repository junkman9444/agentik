...
Split out of ``sage_cli/config.py``; every name is re-imported there, so
``sage_cli.config.<name>`` keeps resolving (and monkeypatching) as before. Functions that call
``get_compatible_custom_providers`` / ``load_config*`` import them lazily from ``sage_cli.config``
...
logger = logging.getLogger("sage_cli.config")
...
def get_custom_provider_model_capability(
    ...
    from sage_cli.config import get_compatible_custom_providers, load_config_readonly
