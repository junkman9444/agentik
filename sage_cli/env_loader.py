...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    if not path.exists():
        return
    try:
        from sage_cli.config import _sanitize_env_lines
    except ImportError:
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and sage_logging reuse one parse instead of 3-4.
...
def _process_hermes_home() -> Path:
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``sage_constants.get_process_hermes_home()`` is the override-immune
