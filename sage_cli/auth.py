...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ...
  re-imported here so ``sage_cli.auth.<name>`` stays the public/patchable surface."""
...
from sage_constants import OPENROUTER_BASE_URL, hermes_home_key, secure_parent_dir
...
from sage_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from sage_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from sage_cli.auth_device_flow import (  # noqa: F401  re-exported
...
from sage_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
# ``sage_cli.auth_nous`` (device-code login, refresh, shared-store mirroring, JWT selection,
# status) no longer exists. Every name below is a safe no-op / fail-closed stand-in so the rest of
# this module (and every consumer that imports these names from ``sage_cli.auth``) keeps working
...
# Additional names ``sage_cli.anon_auth`` (kept; free-tier flow, out of scope for deletion) used
# to import from ``sage_cli.auth_nous`` directly.
...
from sage_cli.auth_minimax import (  # noqa: F401  re-exported
...
from sage_cli.auth_xai import (  # noqa: F401  re-exported
...
from sage_cli.auth_codex import (  # noqa: F401  re-exported
...
from sage_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from sage_cli.auth_qwen import (  # noqa: F401  re-exported
...
from sage_cli.auth_constants import (  # noqa: F401  re-exported
...
def get_anthropic_key() -> str:
    ...
    """
    from sage_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from sage_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    *,
    ...
    with _provider_state_transaction("nous") as (auth_store, state, state_source_path):
        ...
        with _nous_shared_store_lock(timeout_seconds=lock_timeout):
            from sage_cli.anon_auth import is_guest_state, refresh_guest_state
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("sage_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
...
def _provider_is_keyless(provider_id: str) -> bool:
    ...
    try:
        from sage_cli.providers import HERMES_OVERLAYS
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    try:
        from sage_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``sage_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from sage_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "nous":
        from sage_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('sage_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('sage_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('sage_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('sage_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('sage_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NOUS_INFERENCE_INVOKE_SCOPE': ('sage_cli.auth_constants', 'NOUS_INFERENCE_INVOKE_SCOPE'),
    'NOUS_SHARED_STORE_FILENAME': ('sage_cli.nous_compat', '_NOUS_SHARED_STORE_FILENAME_STUB'),
    'OAUTH_OVER_SSH_DOCS_URL': ('sage_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('sage_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('sage_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('sage_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('sage_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('sage_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('sage_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('sage_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('sage_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nous_oauth_pure': ('sage_cli.nous_compat', '_refresh_nous_oauth_pure_stub'),
}
