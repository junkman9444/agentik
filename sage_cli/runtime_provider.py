...
Custom-provider lookup lives in :mod:`sage_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`sage_cli.runtime_provider_backends` — both re-exported here so
``sage_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from sage_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
...
from sage_cli import config as _config_mod
from sage_cli import models as _models  # attribute access keeps ``sage_cli.models.<name>`` patches effective
from sage_constants import OPENROUTER_BASE_URL
from sage_cli.providers import determine_api_mode, is_actual_route, is_official_openai_host, nous_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``sage_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in sage_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in sage_cli/providers.py so the runtime resolver stays in
...
from sage_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from sage_cli.runtime_provider_backends import (  # noqa: E402,F401
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('sage_cli.providers', 'custom_provider_aliases'),
