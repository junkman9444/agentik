...
from sage_cli.pty_session import PtySessionRegistry
...
try:
    if sys.platform.startswith("win"):
        from sage_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    else:
...
def _ws_host_origin_reason(ws: "WebSocket") -> Optional[str]:
    ...
    """
    from sage_cli.web_server import _is_accepted_host, app
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    if auth_required:
        ...
        from sage_cli.dashboard_auth.audit import AuditEvent, audit_log
        from sage_cli.dashboard_auth.ws_tickets import (
...
def _resolve_chat_argv(
    ...
    """
    from sage_cli.web_server_profiles import _config_profile_scope, _resolve_profile_dir
    from sage_cli.web_server_sessions import _open_session_db_for_profile, _session_latest_descendant
    from sage_cli.main import PROJECT_ROOT
    from sage_cli.main_tui_launch import _apply_tui_python_env, _make_tui_argv
...
def _server_internal_ws_url(path: str, **extra_qs) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from sage_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def _resolve_chat_argv_async(
    ...
    from sage_cli.web_server import _get_chat_argv_lock, app
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    from sage_cli.web_server import _get_pty_active_session_files
