...
Split out of ``update_cmd.py``; every name is re-imported there so ``sage_cli.update_cmd.<name>`` keeps
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``sage_cli.main`` re-export surface.
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, sage_constants.py, ...). Check up front
