...
Split out of ``sage_cli.kanban_db``; origin-resident helpers are reached
...
if TYPE_CHECKING:
    from sage_cli.kanban_db import Task
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``sage_cli.profiles.profile_exists``, or ``None`` when it cannot be
...
def _module_hermes_argv() -> list[str]:
    """Interpreter-bound Hermes CLI invocation (``sage_cli.main`` is the
...
def _resolve_hermes_argv() -> list[str]:
    ...
    sage_cli.main`` for shim-less environments (cron, systemd ``User=``,
    launchd). Mirrors ``gateway.run._resolve_hermes_bin``; local because
    ``sage_cli`` sits below ``gateway`` in the dependency order.
    """
    import shutil
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    from sage_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    # sage_constants is imported.
    try:
        env["SAGE_HOME"] = resolve_profile_env(profile_arg)
    ...
    cmd = _worker_argv(task, profile_arg, env.get("SAGE_HOME"))
...
from sage_cli import kanban_db as _kb  # noqa: E402
from sage_cli import kanban_db_connect as _kbc  # noqa: E402
from sage_cli import kanban_db_workspace as _kbw  # noqa: E402
