...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from sage_cli.main import _resolve_last_session
    target = (
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    # sage_cli/tui_dist/entry.js but never ui-tui/ (git checkouts only).
    # 1. A prebuilt install (Docker image, Nix build, or prior `npm run build`) ships
    #   sage_cli/tui_dist/entry.js but never ships ui-tui/ at all (that directory only exists in a git
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sage_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    from sage_cli.shared_session_attach import configure_tui_attachment
    try:
...
def _pin_kanban_board_env() -> None:
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from sage_cli.kanban_db import get_current_board
