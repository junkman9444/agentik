    ...
    for entry in pack.plugins:
        ...
        try:
            if catalog_entries is None:
                from sage_cli.plugin_catalog import load_catalog_live
...
def export_pack(*, enabled_only: bool = False, pack_name: str = "my-hermes-pack") -> tuple[str, List[str]]:
    ...
    import yaml
    from sage_cli.plugins_cmd import _get_enabled_set, _plugins_dir, _read_install_metadata
...
def cmd_pack_install(source: str, *, force: bool = False) -> None:
    ...
    from sage_cli.plugins_cmd import _ask_yes, _console, _fail, _is_tty
    console = _console()
    pack, resolved = _load_and_review(console, source)
...
def pack_command(args) -> None:
    ...
    if handler is None:
        from sage_cli.plugins_cmd import _console, _fail
