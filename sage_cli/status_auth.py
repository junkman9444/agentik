...
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``sage_cli.status``
...
def _render_auth_providers(ctx):
    ...
    if nous_status.get("free_tier"):
        ...
        from sage_cli.anon_auth import FREE_TIER_LABEL, GUEST_MODEL, UPGRADE_HINT
...
def _render_apikey_providers(ctx):
    ...
    if _status._effective_provider_label() == "LM Studio":
        from sage_cli.models_local import probe_lmstudio_models
...
import sage_cli.status as _status  # noqa: E402  (bottom: sage_cli.status imports this module)
