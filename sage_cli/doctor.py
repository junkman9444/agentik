...
from sage_cli.config import get_env_path, get_hermes_home, get_project_root
...
from sage_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from sage_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from sage_cli.doctor_tools import _safe_which

from sage_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from sage_cli.doctor_platform import (
    _check_certificates,
...
from sage_cli.doctor_tools import (
    _check_git_and_rg,
...
from sage_cli.doctor_state import (
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from sage_cli.auth import get_nous_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
    ...
    with warn_on_error(""):  # xAI OAuth separately, so an import failure cannot disrupt the rows already printed
        from sage_cli.auth import get_xai_oauth_auth_status
...
def _ack_advisory(ack_target: str) -> None:
    ...
    from sage_cli.security_advisories import ADVISORIES, ack_advisory
...
def run_doctor(args):
    ...
    with warn_on_error(""):
        from sage_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('sage_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('sage_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('sage_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('sage_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('sage_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('sage_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('sage_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('sage_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('sage_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('sage_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('sage_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('sage_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('sage_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('sage_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('sage_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('sage_cli.doctor_config', 'report_deprecated_config_and_env'),
}
