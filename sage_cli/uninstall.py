...
def remove_wrapper_script():
    ...
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a sage_cli / hermes-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('sage_cli' in content or 'hermes-agent' in content, wrapper)
...
def uninstall_gateway_service():
    ...
    try:
        from sage_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from sage_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from sage_cli import gateway_windows as gw
...
def remove_hermes_env_vars_windows() -> list[str]:
    ...
    def edit(winreg, key, removed):
        for name in ("SAGE_HOME", "HERMES_GIT_BASH_PATH"):
            try:
...
def _uninstall_profile(profile) -> None:
    ...
    # 1. Gateway service, via `python -m sage_cli.main` (the `hermes` wrapper may be half-gone).
    hermes_invocation = [sys.executable, "-m", "sage_cli.main", "--profile", name]
...
def run_gui_uninstall(args):
    ...
    from sage_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
...
def _perform_uninstall(
    *,
    ...
    try:
        from sage_cli.gui_uninstall import uninstall_gui
...
def main(argv=None) -> int:
    """``python -m sage_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``sage_constants`` + ``sage_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m sage_cli.uninstall")
    parser.add_argument(
