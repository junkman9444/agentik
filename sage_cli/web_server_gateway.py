...
from sage_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from sage_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from sage_cli.profiles import _check_gateway_running, profiles_to_serve
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from sage_cli.web_server import PROJECT_ROOT
...
def _spawn_hermes_action(
    ...
    """Spawn ``hermes <subcommand>`` detached (via ``sage_cli.main``) and record the handle."""
    from sage_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "sage_cli.main", *subcommand]
...
def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    from sage_cli.web_server_profiles import _profile_cli_args
