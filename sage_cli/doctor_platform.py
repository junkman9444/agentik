...
Split out of ``sage_cli/doctor.py``, which re-exports every name so ``sage_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from sage_cli.config import is_nix_install_method, recommended_update_command_for_method
from sage_cli.doctor_report import (
    Finding, _fail_and_issue, _section, check_bool, check_fail, check_info, check_ok, check_warn, doctor_check,
    warn_on_error,
)
from sage_constants import is_termux as _is_termux
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from sage_cli.doctor import PROJECT_ROOT
...
def _hermes_database_paths(hermes_home: Path) -> list[tuple[str, Path]]:
    ...
    from sage_cli.backup import _QUICK_STATE_FILES
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``sage_cli.sqlite_safe_read``), and the dashboard console runs
    ``run_doctor`` in-process with live ``SessionDB`` connections — the helper refuses then (unreadable).
    """
    from sage_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
...
def _report_database_journal_modes(hermes_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from sage_cli.doctor import HERMES_HOME
    from sage_state_wal import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs sage_cli.__version__ drift (a conflict resolution can revert one but not the
    other). Silent no-op for installed wheels (no pyproject)."""
    try:
        from sage_cli import __version__ as init_version
    except Exception:
        return
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != sage_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'hermes update', or set sage_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from sage_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from sage_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from sage_cli import macos_tcc_anchor as tcc
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from sage_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from sage_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
