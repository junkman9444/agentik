...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    from sage_cli.setup import get_env_value
    try:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    detection), else None. ``get_env_value`` is reached through sage_cli.setup so test patches
...
def _skip_configured_section(config: dict, section_key: str, label: str) -> bool:
    ...
    from sage_cli.setup import print_success, prompt_yes_no
...
def _print_migration_preview(report: dict):
    ...
    config semantics)."""
    from sage_cli.setup import color, Colors, print_info
...
def _migration_step(label: str, log_label: str, fn):
    ...
    from sage_cli.setup import print_warning
    try:
        return fn()
