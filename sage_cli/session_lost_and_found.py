...
from sage_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts
...
# 3.50.7 and 3.44.6 — the same version gate sage_state applies to the
...
#
# The predicate lives in sage_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from sage_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502
...
# The closed sets SessionDB writes (sage_state.py: request/claim/complete/
