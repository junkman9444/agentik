...
Callers: ``_early_recovery.recover_if_needed`` (stdlib-only, runs BEFORE ``sage_cli.main``'s
third-party imports so a pending update completes while no native extension is mapped yet) and
``sage_cli.main_install_repair._recover_core_update_marker_locked`` (the post-import recovery path).
...
from sage_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (sage_cli.main's version lives behind imports)."""
...
def _venv_scripts_dir(root: Path) -> Path | None:
    """Project venv Scripts/bin dir, when present (sage_constants is stdlib-only)."""
    # sage_constants is stdlib-only, so the canonical layout helpers are safe to use from this
    # corrupted-venv repair path (#76105: never open-code the Scripts/bin split).
    from sage_constants import project_venv_dir, venv_bin_dir
...
def ensure_windows_bin_launchers(
    ...
    :func:`sage_constants.get_default_hermes_root`. See #83797.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    targets: list[Path] = []
    # Runs at every sage_cli.main process start, so the healthy path must stay a few stat calls.
...
def migrate_windows_bin_path(
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from sage_constants import venv_bin_dir
