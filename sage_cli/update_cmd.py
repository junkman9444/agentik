...
``sage_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``sage_cli.main`` at call time.
"""
...
from sage_cli.config import get_hermes_home  # noqa: F401  (re-exported; patched via update_cmd)
from sage_cli.update_cmd_common import _best_effort
from sage_constants import get_default_hermes_root, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from sage_cli.update_abort_recovery import (  # noqa: F401
...
from sage_cli.update_cmd_windows import (  # noqa: F401
...
from sage_cli.update_cmd_fleet import (  # noqa: F401
...
from sage_cli.update_cmd_zip import (  # noqa: F401
...
from sage_cli.update_cmd_stash import (  # noqa: F401
...
from sage_cli.update_cmd_config import (  # noqa: F401
...
from sage_cli.update_cmd_deps import (  # noqa: F401
...
from sage_cli.update_cmd_git import (  # noqa: F401
...
from sage_cli.update_cmd_maint import (  # noqa: F401
...
def _m():
    """Lazy ``sage_cli.main`` handle: keeps main-side test patches effective, import one-way."""
...
_UPDATE_CRITICAL_FILES = (
    "sage_cli/main.py", "sage_cli/config.py", "sage_cli/__init__.py",
    "sage_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
    "sage_constants.py")
...
def _record_update_step(step: str, ok: bool, detail: str = "") -> None:
    ...
    with suppress(Exception):
        from sage_cli.update_receipt import record_step
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from sage_constants import get_hermes_home  # noqa: F811  (deliberate: constants variant)
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from sage_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from sage_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    from sage_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    from sage_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            return
        from sage_cli.banner import _github_compare_behind
...
def _repair_current_checkout(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, desktop_dir,
    ...
    from sage_cli.managed_uv import ensure_uv, update_managed_uv
    runtime_repairs = []
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in sage_cli/config.py bricked
...
def _begin_update_receipt_and_plan(args):
    ...
    with _best_effort('Update receipt unavailable: %s'):
        ...
        from sage_cli.update_receipt import begin_update_receipt
    ...
    with _best_effort('Update plan phase failed: %s'):
        from sage_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            if _m()._is_windows():
                ...
                print(
                    ...
                    '"from sage_cli.main import main; main()" update --yes')
