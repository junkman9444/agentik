...
Split out of ``sage_cli/main.py``. Names that still live in main are imported lazily at call time.
"""

import contextlib

from typing import Optional
from sage_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    ...
    from sage_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`sage_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_select_for_task(task: str) -> None:
    ...
    from sage_cli.config import load_config
    from sage_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    with contextlib.suppress(Exception):
        from sage_cli.setup import _curses_prompt_choice
...
def _stepfun_base_url_for_region(region: str) -> str:
    from sage_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from sage_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            from sage_constants import display_hermes_home as _dhh_fn
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from sage_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    from sage_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from sage_cli.models_catalog_static import group_providers, provider_group_for_slug
