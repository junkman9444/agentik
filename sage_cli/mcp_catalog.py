...
from sage_constants import get_hermes_home, get_optional_mcps_dir
from sage_cli._subprocess_compat import noninteractive_git_env
from sage_cli.colors import Colors, color
from sage_cli.config import load_config, save_config, get_env_value, save_env_value
from sage_cli.cli_output import prompt as _prompt_input
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by sage_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from sage_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from sage_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    from sage_cli.mcp_config import _save_mcp_server
