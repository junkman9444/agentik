...
gateway, sessions, …) is built by ``sage_cli/subcommands/<group>.py`` and wired in
...
# live parser fails. The derived path is authoritative; tests/sage_cli/
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``sage_cli.relaunch``."""
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    unless the flag is actually passed after the subcommand (tests/sage_cli/
