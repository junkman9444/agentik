...
def _run_fresh_recovery_process(
    ...
    """Spawn ``sage_cli.update_restart_recovery --stdin`` detached from this process; None when it
    could not run (no systemd-run in gateway mode, OSError, timeout) — the caller fails closed."""
    command = [sys.executable, "-m", "sage_cli.update_restart_recovery", "--stdin"]
    env = os.environ.copy()
...
def _recover_gateway_restart_after_abort(
    ...
    """
    from sage_cli.update_cmd import _gateway_recovery_partition
