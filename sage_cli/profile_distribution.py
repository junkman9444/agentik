...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "sage_state.db", "response_store.db",
...
def plan_install(source: str, workdir: Path, override_name: Optional[str] = None) -> InstallPlan:
    ...
    from sage_cli.profiles import _canon_valid, get_profile_dir
...
def _bootstrap_user_dirs(target: Path) -> None:
    ...
    from sage_cli.profiles import _PROFILE_DIRS
...
def install_distribution(
    ...
    from sage_cli.profiles import check_alias_collision, create_wrapper_script
...
def _existing_profile(profile_name: str) -> Tuple[str, Path]:
    ...
    from sage_cli.profiles import _existing_profile_dir
