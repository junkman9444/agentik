...
TIPS = [
    ...
    "Ctrl+Z suspends Sage to the background — run fg in your shell to resume.",
    ...
    "sage chat -t web,terminal enables only specific toolsets for a focused session.",
    "sage chat -s github-pr-workflow preloads a skill at launch.",
    "sage chat -q \"query\" runs a single non-interactive query and exits.",
    "sage chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "sage chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "sage chat --source telegram tags the session for filtering in sage sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "sage doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "sage dump outputs a compact setup summary — great for bug reports.",
    "sage config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "sage config edit opens config.yaml in your default editor.",
    "sage config check scans for missing or stale configuration options.",
    "sage sessions browse opens an interactive session picker with search.",
    "sage sessions stats shows session counts by platform and database size.",
    "sage sessions prune --older-than 30 cleans up old sessions.",
    "sage skills search react --source skills-sh searches the skills.sh public directory.",
    "sage skills check scans installed hub skills for upstream updates.",
    "sage skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "sage skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "sage mcp add github --command npx adds MCP servers from the command line.",
    "sage mcp serve runs Sage itself as an MCP server for other agents.",
    "sage auth add lets you add multiple API keys for credential pool rotation.",
    "sage completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "sage logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "sage backup creates a zip backup of your entire Sage home directory.",
    "sage profile create coder creates an isolated profile that becomes its own command.",
    "sage profile create work --clone copies your current config and keys to a new profile.",
    "sage update syncs new bundled skills to ALL profiles automatically.",
    "sage gateway install sets up Sage as a system service (systemd/launchd).",
    "sage memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "sage webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: sage tools disables unused tools, sage skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "sage models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    # --- Configuration ---
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Sage tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'sage profile create coder' creates the 'coder' command.",
    "sage profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Sage prints a resume command with session ID and stats.",
    "sage sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "sage skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: sage cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Sage runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "sage gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Sage via a local macOS server.",
    ...
    "Sage loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "sage mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "sage mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "sage mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — sage tools can toggle them per platform.",
    ...
    "sage chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "sage plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "sage acp runs Sage as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "sage auth add nous or sage auth add openai-codex sets up OAuth-based providers.",
    ...
    "sage status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Sage your own.",
    ...
    "Before a gateway session reset, Sage auto-flushes important facts to memory in the background.",
    ...
    "Profiles scope Sage state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'sage profile create ls' would be rejected.",
    "sage profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "sage claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via sage tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Sage auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Sage config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Sage is working.',
    ...
    # --- Curator ---
    'sage curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "sage curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'sage curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',
    # --- Credential Pools & Routing ---
    'sage auth reset <provider> [target] clears cooldowns and exhaustion flags on a whole credential pool, or on one credential by index, id, or label.',
    ...
    'sage dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'sage dashboard embeds the full Sage TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'sage chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'sage chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'sage chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "sage chat --source tool tags programmatic chats so they don't clutter sage sessions list.",
    'sage dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'sage sessions rename <ID> "new title" renames any past session; sage sessions delete <ID> removes one.',
    'sage import restores a full Sage backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'sage fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'sage pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'sage setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'sage status --deep runs the full health sweep across every component; plain sage status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Sage block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from sage sessions list by default — set --source explicitly to see them.',
...
# Task-oriented example prompts for the empty composer. Kept generic — Sage is
