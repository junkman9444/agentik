...
disk cache). Split out of ``sage_cli.models``; helpers still defined there are looked up on
``sage_cli.models`` at call time so ``patch("sage_cli.models.<name>")`` mocks keep intercepting.
"""
...
from sage_cli.urllib_security import url_origin

# Log-record parity with the origin module.
logger = logging.getLogger("sage_cli.models")
...
def _get_ollama_base_url() -> str:
    ...
    from sage_cli.models import _get_model_config_dict
...
def probe_ollama_local_models(
    ...
    from sage_cli.models import _HERMES_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from sage_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    from sage_cli.models import _HERMES_USER_AGENT
    from agent.command_token_source import materialize_probe_api_key
...
def _lmstudio_fetch_raw_models(
    ...
    from sage_cli.models import _urlopen_model_catalog_request
...
def _load_ollama_cloud_cache(*, ignore_ttl: bool = False) -> Optional[dict]:
    ...
    from sage_cli.models import _read_json_cache
...
def _save_ollama_cloud_cache(models: list[str]) -> None:
    ...
    from sage_cli.models import _write_json_cache
