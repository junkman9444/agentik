...
from sage_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
from sage_constants import clear_named_profile_deleted, mark_named_profile_deleted, named_profile_is_deleted
...
# stay visible; containers persist HOME state here). See sage_constants.get_subprocess_home().
...
# when introduced in ``sage_constants``.
# See #58394.
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    with contextlib.suppress(Exception):
        ...
        from sage_cli.config import check_config_version, migrate_config
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from sage_cli.gateway import named_profile_served_by_running_multiplexer
...
def create_profile(
    ...
    with contextlib.suppress(Exception):  # best-effort — don't fail profile creation over this
        from sage_cli.default_soul import DEFAULT_SOUL_MD
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SAGE_HOME": str(profile_dir)},
...
_HERMES_ARGV_MARKERS = ("sage_cli.main", "hermes-gateway", "tui_gateway")
        ...
        try:
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("SAGE_HOME", "")
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    old_home = os.environ.get("SAGE_HOME")
    try:
        os.environ["SAGE_HOME"] = str(profile_dir)
        from sage_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        os.environ.pop("SAGE_HOME", None)
        if old_home is not None:
            os.environ["SAGE_HOME"] = old_home
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.hermes (dir name ".sage"), so both paths stage a filtered
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    env_home = os.environ.get("SAGE_HOME", "").strip()
    if env_home:
