...
from sage_constants import display_hermes_home, is_termux as _is_termux_environment
from agent.turn_context import extract_api_content_sidecar
from sage_cli.browser_connect import (
...
class _TTYBuf(StringIO):
    """StringIO that claims to be a TTY so ``sage_cli.colors.color()`` still emits ANSI escapes."""
...
def _print_side_result_panel(cli, *, header_lines, body, title_suffix, empty_note, console=None) -> None:
    ...
    try:
        ...
        label = _skin.get_branding("response_label", "⚕ Sage")
        _resp_color = _maybe_remap_for_light_mode(_skin.get_color("response_border", "#CD7F32"))
        _resp_text = _maybe_remap_for_light_mode(_skin.get_color("banner_text", "#FFF8DC"))
    except Exception:
        label, _resp_color, _resp_text = "⚕ Sage", "#CD7F32", "#FFF8DC"
...
class CLICommandsMixin:
    ...
    def _snapshot_list(self, parts) -> None:
        from sage_cli.backup import list_quick_snapshots
    ...
    def _snapshot_restore(self, parts) -> None:
        from sage_cli.backup import list_quick_snapshots, restore_quick_snapshot
        if len(parts) < 3:
    ...
    def _snapshot_prune(self, parts) -> None:
        from sage_cli.backup import prune_quick_snapshots
        keep = 20
        if len(parts) > 2:
            try:
                keep = int(parts[2])
    ...
    def _handle_export_command(self, command: str):
        ...
        from sage_cli.profiles import export_profile, get_active_profile_name, get_profile_export_path
    ...
    def _handle_import_command(self, command: str):
        ...
        new profile."""
        from sage_cli.profiles import check_alias_collision, create_wrapper_script, import_profile
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        ...
        from sage_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from sage_cli.clipboard import has_clipboard_image
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from sage_cli.clipboard import is_remote_shell_session, write_clipboard_text
    ...
    def _run_tools_config(self, **ns) -> None:
        ...
        from sage_cli.tools_config import tools_disable_enable_command
    ...
    def _resolve_resume_target(self, target: str):
        ...
        if target.isdigit():
            ...
            from sage_cli.main import _resolve_session_by_name_or_id
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        persistence goes through sage_cli.personality, the single owner of personality state."""
        from sage_cli.personality import (
    ...
    def _handle_curator_command(self, cmd: str):
        """Handle /curator — delegates to sage_cli.curator so the CLI and the `hermes curator`
        subcommand share the same handler set."""
        tokens = shlex.split(cmd)[1:] if cmd else []
        try:
            from sage_cli.curator import cli_main
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to sage_cli.skills_hub, after intercepting the
        ...
        if args and args[0].lower() in review_words:
            ...
            if out is not None:
                return print(out)
        from sage_cli.skills_hub import handle_skills_slash
    ...
    def _handle_login_command(self, cmd_original: str) -> None:
        """`/login`. Nous free-tier sign-in has been removed from this fork -- always dead at runtime
        anyway (anon_auth.run_sign_in's own precondition check returns Unavailable immediately,
        since guest_enabled() is gated on HERMES_GUEST_ONBOARDING=1, which nothing in this fork
        sets), but the command now says so immediately instead of spinning up the sign-in thread to
        reach the same answer."""
        print("  🔴 Free-tier sign-in is not available in this fork.")
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        from sage_cli.heartbeat import format_interval
        arg = _command_arg(cmd)
        lower = arg.lower()
    ...
    def _heartbeat_set(self, mgr, arg: str) -> None:
        ...
        from sage_cli.heartbeat import parse_interval, format_interval
        ...
        if interval < 0:
            from sage_cli.heartbeat import MIN_INTERVAL_SECONDS
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        from sage_cli.goals import last_user_message_content
    ...
    def _handle_loop_command(self, cmd: str) -> None:
        ...
        if mgr is None:
            return
        from sage_cli.loops import dispatch_loop_command
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from sage_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_focus_command(self, cmd_original: str) -> None:
        ...
        from sage_cli.colors import Colors as _Colors
    ...
    def _set_tool_progress_mode(self, mode: str) -> None:
        ...
        from sage_cli.focus_view import normalize_tool_progress_mode
    ...
    def _note_focus_hidden_line(self, function_name: str) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from sage_cli.focus_view import would_display_tool_line
    ...
    def _emit_focus_recovery_line(self) -> None:
        ...
        if not getattr(self, "_focus_view_enabled", False):
            return
        from sage_cli.focus_view import format_hidden_line
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        anthropic = _probe("sage_cli.models", "_is_anthropic_fast_model", None, model)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        from sage_cli.debug import run_debug_share
