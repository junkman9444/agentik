...
Split out of ``sage_cli/model_switch.py``; every moved name is re-imported there so
``sage_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from sage_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("sage_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from sage_cli.model_switch import _extra_headers_from_config
...
def _fetch_picker_live_models(
    ...
    from sage_cli.models import _get_ollama_native_headers, cached_fetch_api_models, fetch_api_models
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from sage_cli.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from sage_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    ...
    from sage_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from sage_cli.models import _AGGREGATOR_PROVIDERS
    from sage_cli.providers import ALIASES
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    ...
    from sage_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from sage_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from sage_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from sage_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from sage_cli.auth import PROVIDER_REGISTRY
...
class _PickerBuild:
    ...
    imports of ``sage_cli.auth/models`` stay lazy so tests can patch those modules."""
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from sage_cli.model_switch import _declared_model_ids
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if hermes_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            from sage_cli.models import cached_provider_model_ids
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from sage_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from sage_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from sage_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from sage_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    ...
    from sage_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nous_model_ids
...
def list_authenticated_providers(
    ...
    from sage_cli.config import coerce_provider_id, stringify_provider_map
    ...
    if current_model:
        for row in results:
            ...
            if current_model not in models:
                from sage_cli.models import _model_requires_account_discovery
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from sage_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`sage_cli.models.fetch_openrouter_models` (curated
    ...
    from sage_cli.model_switch import list_authenticated_providers
    from sage_cli.models import fetch_openrouter_models
