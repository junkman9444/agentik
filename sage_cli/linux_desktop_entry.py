...
def _can_import_hermes_cli(interpreter: Path) -> bool:
    """Whether *interpreter* can import ``sage_cli.main`` unaided.
...
def _running_interpreter_fallback() -> str:
    """The RUNNING interpreter — it has ``sage_cli`` importable by definition."""
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    ...
    Prefer the real ``hermes`` launcher; fall back to ``<python> -m sage_cli.main desktop``.
    """
    from sage_cli.relaunch import resolve_hermes_bin
    ...
    if not _can_import_hermes_cli(Path(interpreter)):
        ...
        # The candidate interpreter cannot actually import sage_cli.main (checked in isolated mode from a
    ...
    argv = [interpreter, "-m", "sage_cli.main", "desktop"]
    if bin_path:
...
def _inside_checkout(candidate: str, checkout_root: Path, original_argv0: str) -> bool:
    ...
    for root in {checkout_root, resolved_root}:
        if root is not None and (path == root or root in path.parents):
            return True
    # The `python -m sage_cli.main` relaunch context surfaces the invoking interpreter as
...
def _resolve_hermes_bin_for_desktop_entry(
    ...
    Wraps :func:`sage_cli.relaunch.resolve_hermes_bin` with one rule: an ``argv[0]`` inside this
    ...
    if resolve_fn is None:
        from sage_cli.relaunch import resolve_hermes_bin as resolve_fn
