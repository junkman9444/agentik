...
from sage_cli.tools_config_cua import (
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung sage_cli.doctor uses so this probe can't diverge
...
def _post_setup_spotify() -> None:
    ...
    try:
        from sage_cli.auth import login_spotify_command
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        from sage_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from sage_cli.setup import prompt_choice, prompt as _setup_prompt
        from sage_cli.setup_tts import _run_xai_oauth_login_from_setup
...
def _agent_browser_installed() -> bool:
    ...
    *and* the Chromium build it drives (or the Lightpanda engine, which needs no Chromium), so "Run
    ...
    from sage_cli.nous_compat import _local_browser_runnable
...
def _cloud_agent_browser_installed() -> bool:
    ...
    from sage_cli.nous_compat import _has_agent_browser
