...
def process_state(pid: int, creation_time_ns: int, hermes_path: str, spawn_nonce: str) -> dict[str, Any]:
    ...
    launches_module = (
        argv[1:3] == ["-m", "sage_cli.main"]
        or (len(argv) > 2 and argv[1] == "-c" and "sage_cli.main" in argv[2]))
...
def _resolve_direct_interpreter(python_entry: str) -> tuple[str, list[str]]:
    ...
    process that both owns the port and is the one we lock. sage_cli's parent is prepended
    ...
    if not root or not os.path.isdir(root):
        raise ValueError("could not locate the sage_cli package")
...
def spawn_backend(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    bootstrap = (
        ...
        "runpy.run_module('sage_cli.main',run_name='__main__',alter_sys=True)")
