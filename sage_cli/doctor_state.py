...
from sage_cli.sizefmt import format_bytes as _human_bytes
from sage_state_common import FTS_STORAGE_VERSION
...
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    ...
    from sage_cli.doctor import HERMES_HOME, _DHH
...
def _state_db_health(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str) -> None:
    ...
    try:
        ...
        from sage_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from sage_state_dbfile import collect_state_db_stats, count_db_holders
...
def _check_profiles(should_fix: bool, f: Finding) -> None:
    from sage_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
    import re as _re
