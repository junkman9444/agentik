...
Imported by ``sage_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
sage_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
def print_fast_version_info(*, check_updates: bool = False) -> None:
    ...
    try:
        ...
        from sage_cli import __release_date__, __version__
    ...
    try:
        from sage_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
