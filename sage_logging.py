...
from sage_constants import get_config_path, get_hermes_home, mkdir_under_hermes_home
...
COMPONENT_PREFIXES = {
    ...
    "cli": ("sage_cli", "cli"),
    "cron": ("cron",),
    "gui": ("sage_cli.web_server", "sage_cli.pty_bridge", "tui_gateway", "uvicorn"),
}
...
def _read_logging_config():
    ...
    try:
        ...
        # sage_cli.main's early parse (one config.yaml parse per process);
        # fall back to a direct parse for bare sage_logging consumers.
        try:
            from sage_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
