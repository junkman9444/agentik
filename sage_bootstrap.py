...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``sage_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``sage_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
# Apply on import — entry points only need ``import sage_bootstrap`` first.
