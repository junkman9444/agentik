...
# 5. Symlinks the 'sage' CLI command (and 'hermes' compat alias) into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/sage-uv-install.$$.log")"
        _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sage-uv-installer.$$.sh")"
...
# ============================================================================
# PATH setup — symlink sage (and the hermes compat alias) into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up sage command..."

SAGE_BIN="$SCRIPT_DIR/venv/bin/sage"
HERMES_BIN="$SCRIPT_DIR/venv/bin/hermes"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SAGE_BIN" "$COMMAND_LINK_DIR/sage"
ln -sf "$HERMES_BIN" "$COMMAND_LINK_DIR/hermes"
echo -e "${GREEN}✓${NC} Symlinked sage → $COMMAND_LINK_DISPLAY_DIR/sage (hermes alias also linked)"
...
# ============================================================================
# Seed bundled skills into ~/.sage/skills/
# ============================================================================

SAGE_SKILLS_DIR="${SAGE_HOME:-${HERMES_HOME:-$HOME/.sage}}/skills"
mkdir -p "$SAGE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.sage/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SAGE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  sage status        # Check configuration"
if is_termux; then
    echo "  sage gateway       # Run gateway in foreground"
else
    echo "  sage gateway install # Install gateway service (messaging + cron)"
fi
echo "  sage cron list     # View scheduled jobs"
echo "  sage doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m sage_cli.main setup
fi
