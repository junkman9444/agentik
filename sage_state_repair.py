"""state.db repair, backup and writability preflight (split from sage_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``sage_state_repair.<name>``.
"""
...
from sage_startup_watchdog import report_startup_progress
...
# Log-record parity with the origin module (caplog tests pin "sage_state").
logger = logging.getLogger("sage_state")
...
def _read_offline(db_path: Path, what: str, reader) -> Optional[str]:
    """Run *reader()* under ``sage_cli.sqlite_safe_read.offline_file_access``.
    ...
    without sage_cli have no tracked connections."""
    try:
...
def _claim_repair_attempt(db_path: Path) -> bool:
    ...
    from sage_state import _repair_attempt_lock, _repair_attempted_paths
...
def _try_lock_nonblocking(handle) -> None:
    ...
    from sage_state import _IS_WINDOWS
    if _IS_WINDOWS:
...
def _cross_process_repair_lock(db_path: Path):
    ...
    #36644).
    """
    from sage_state import _IS_WINDOWS, _REPAIR_LOCK_TIMEOUT_SECONDS
...
def _repair_snapshot_timeout_seconds(source_path: Path) -> float:
    ...
    from sage_state import _REPAIR_LOCK_TIMEOUT_SECONDS
    source_bytes = 0
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    ``close()`` a descriptor and cancel that connection's POSIX advisory locks (``sage_cli.sqlite_safe_read``) —
    ...
    See #69603.
    """
    with contextlib.suppress(ImportError):  # scaffold/embed installs without sage_cli track no connections
...
def _reapply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from sage_state_wal import _apply_macos_checkpoint_barrier, _enforce_macos_synchronous_full
    try:
...
def apply_durability_barriers(conn: sqlite3.Connection) -> bool:
    ...
    from sage_state_wal import _apply_synchronous_pragma
    ok = _reapply_durability_barriers(conn)
    with contextlib.suppress(Exception):
        from sage_cli.config import cfg_get, load_config_readonly  # local: avoids an import cycle
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    See #50502.
    """
    from sage_state import SessionDB, load_fts5_cjk_extension
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    (``sage_state_holders``) fails closed on deleted-WAL-generation, uninspectable, or unknown holders."""
...
def _probe_journal_mode_for_repair(db_path: Path) -> Optional[str]:
    ...
    from sage_state_wal import _on_disk_journal_mode
    try:
...
def _strategy_rebuild_fts(conn: sqlite3.Connection) -> None:
    ...
    from sage_state import load_fts5_cjk_extension
