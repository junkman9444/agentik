...
ORIGINAL_HOME = os.environ.get("SAGE_HOME")
ORIGINAL_AUTH = Path.home() / ".sage" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    real_env_file = Path.home() / ".sage" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".sage"))
...
def main():
    ...
    if ORIGINAL_HOME is not None:
        os.environ["SAGE_HOME"] = ORIGINAL_HOME
    else:
