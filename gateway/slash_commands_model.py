...
from sage_cli.config import atomic_config_write, clear_model_endpoint_credentials
...
async def _persist_model_switch_to_config(result, config_path) -> None:
    ...
    """
    from sage_cli.config import read_user_config_raw, save_config
...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        try:
            from sage_cli.context_switch_guard import enrich_model_switch_warnings_for_gateway
    ...
    async def _record_model_switch(
        ...
        from sage_cli.model_switch import format_model_for_display
    ...
    async def _model_switch_confirmation(
        ...
        from gateway.run import _load_gateway_config
        from sage_cli.model_switch import format_model_for_display, resolve_display_context_length_async
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from sage_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
        ...
        ctx = _ModelSwitchContext(
            ...
            # the caller's). user_id is critical for the fallback lookup path (sage_state.py:1994-2009)
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        from sage_cli import codex_runtime_switch as crs
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality — list or set a personality (sage_cli.personality owns the state)."""
        from gateway.run import _load_gateway_config
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from sage_cli.models import model_supports_fast_mode
