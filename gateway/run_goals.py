...
class GatewayGoalsMixin:
    ...
    def _goal_max_turns_from_config(self) -> int:
        ...
        top-level ``goals`` block is only reachable via sage_cli.config.load_config()."""
        try:
            goals_cfg = (
    ...
    async def _warm_goals_session_db(self, label: str) -> None:
        ...
        try:
            from sage_cli.goals import _get_session_db as _warm_goals_db
    ...
    def _start_heartbeat_poller(self) -> None:
        ...
        from sage_cli.heartbeat import POLL_SECONDS
    ...
    async def _post_turn_goal_continuation(
        ...
        with suppress(Exception):
            from sage_cli.goals import count_active_delegations, gather_background_processes as _gather_bg
    ...
    async def _loop_wakeup_watcher(self, interval: float = 15.0) -> None:
        ...
        async def _scan_one_store(profile_name: Optional[str]) -> None:
            from sage_cli.loops import list_active_loops
