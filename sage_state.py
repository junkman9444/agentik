...
from sage_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from sage_state_errors import (
...
from sage_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from sage_state_sessions import SessionSessionsMixin
from sage_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from sage_state_portability import SessionPortabilityMixin
from sage_state_telegram import SessionTelegramTopicsMixin
from sage_state_schema import SessionSchemaMixin
import sage_state_holders as _state_holders
from sage_state_dbfile import (
...
from sage_state_messages import SessionMessagesMixin
from sage_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
from sage_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from sage_state_titles import SessionTitlesMixin
from sage_state_usage import SessionUsageMixin
from sage_state_maintenance import SessionMaintenanceMixin
from sage_state_gateway import SessionGatewayMixin
from sage_state_compression import SessionCompressionMixin
...
# Live-DB guard knobs live HERE (not in sage_state_guard): the hermetic conftest monkeypatches
# ``sage_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
...
# connection per resolved path via sage_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in sage_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via sage_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via sage_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via sage_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _settle_lost_generation_locked(self) -> bool:
        ...
        through sage_state_registry.release_or_close, which swallows the error, so an interpreter
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see sage_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``sage_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from sage_state_registry import release
            release(self)
            return
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('sage_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('sage_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('sage_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('sage_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('sage_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('sage_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('sage_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('sage_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('sage_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('sage_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('sage_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('sage_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('sage_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('sage_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('sage_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('sage_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('sage_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('sage_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('sage_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('sage_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('sage_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('sage_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('sage_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('sage_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('sage_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('sage_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('sage_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('sage_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('sage_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('sage_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('sage_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('sage_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('sage_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('sage_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('sage_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('sage_state_registry', 'release_or_close'),
    'report_startup_progress': ('sage_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('sage_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('sage_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('sage_state_wal', 'sqlite_source_id'),
    'workspace_key': ('sage_state_sessions', 'workspace_key'),
}
