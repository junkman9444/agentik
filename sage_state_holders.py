...
an unlinked SQLite DB/WAL/SHM generation.  ``sage_state`` supplies only the
...
_HERMES_PYTHON_MODULES = frozenset({"acp_adapter", "sage_cli.main"})
_HERMES_PYTHON_SCRIPTS = frozenset({"sage_cli/main.py", "run_agent.py"})
