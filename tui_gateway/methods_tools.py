    ...
    def deco(body):
        def handler(rid, params: dict) -> dict:
            ...
            if profile := _str_arg(params, "profile") if scoped else "":
                try:
                    profile_dir = _tools_mod("sage_cli.profiles").get_profile_dir(profile)
                    if not profile_dir or not profile_dir.is_dir():
                        return _err(rid, 4064, f"profile '{profile}' not found")
                    token = _tools_mod("sage_constants").set_hermes_home_override(str(profile_dir))
...
def _mcp_named_server(rid, params):
    ...
    name, servers = _str_arg(params, "name"), _tools_mod("sage_cli.mcp_config")._get_mcp_servers()
...
def _capture_run_kwargs(timeout: int) -> dict:
    ...
    return dict(
        ...
        stdin=subprocess.DEVNULL, creationflags=_tools_mod("sage_cli._subprocess_compat").windows_hide_flags())
...
_SIMPLE_RPCS = {
    ...
    "reload.env": (5015, lambda params: {"updated": int(_tools_mod("sage_cli.config").reload_env())}),
    "plugins.list": (5032, lambda params: {"plugins": [
        {"name": n, "version": getattr(i, "version", "?"), "enabled": getattr(i, "enabled", True)}
        for n, i in _tools_mod("sage_cli.plugins").get_plugin_manager()._plugins.items()]}),
...
def _mcp_reload_confirm_required() -> bool:
    ...
    try:
        cfg = _tools_mod("sage_cli.config").load_config()
...
def _catalog_registry(cat: _Catalog) -> None:
    commands = _tools_mod("sage_cli.commands")
...
def _catalog_plugin_commands(cat: _Catalog) -> None:
    plugin_cmds = _tools_mod("sage_cli.plugins").get_plugin_commands() or {}
    if plugin_cmds:
...
def _(rid, params: dict) -> dict:
    ...
    return _ok(rid, {
        "pairs": cat.pairs, "sub": {k: v[:] for k, v in _tools_mod("sage_cli.commands").SUBCOMMANDS.items()},
        "canon": cat.canon,
...
@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m sage_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    return _captured_exec(
        rid, [sys.executable, "-m", "sage_cli.main", *argv], min(int(params.get("timeout", 240)), 600),
...
def _(rid, params: dict) -> dict:
    r = _tools_mod("sage_cli.commands").resolve_command(params.get("name", ""))
    if r:
...
def _plugin_command_handler(name: str):
    try:
        return _tools_mod("sage_cli.plugins").get_plugin_command_handler(name)
    except Exception:
        return None


def _run_plugin_command(handler, arg: str) -> str:
    return str(_tools_mod("sage_cli.plugins").resolve_plugin_command_result(handler(arg)) or "")
...
def _bundle_key_for(name: str):
    ...
    try:
        if _tools_mod("sage_cli.commands").resolve_command(name) is None:
...
_cmd_init = _prompt_builtin("sage_cli.init_command", "build_init_prompt_for_cwd", kw="extra")
...
def _cmd_moa(rid, params, session, name, arg):
    ...
    try:
        moa = _tools_mod("sage_cli.moa_config")
        if not arg:
...
def _cmd_focus(rid, params, session, name, arg):
    ...
    fv = _tools_mod("sage_cli.focus_view")
...
def _cmd_goal(rid, params, session, name, arg):
    with _session_profile_runtime_scope(session or {}):
        sid_key, goals, err = _session_key_or_err(rid, session, "sage_cli.goals", "goals")
        if err:
            return err
        try:
        ...
        from sage_cli.goal_command import dispatch_goal_command
...
def _cmd_loop(rid, params, session, name, arg):
    sid_key, loops, err = _session_key_or_err(rid, session, "sage_cli.loops", "loops")
    if err:
        return err
...
def _configure_session_tools(rid, params: dict, sid: str, session) -> dict:
    ...
    hc, tc = _tools_mod("sage_cli.config"), _tools_mod("sage_cli.tools_config")
    cfg = hc.load_config()
...
def _skills_install(rid, params, query):
    ...
    _tools_mod("sage_cli.skills_hub").do_install(query, skip_confirm=True, console=quiet)
    return _ok(rid, {"installed": True, "name": query})


def _skills_browse(rid, params, query):
    pg = int(params.get("page", 0) or 0) or (int(query) if query.isdigit() else 1)
    browse = _tools_mod("sage_cli.skills_hub").browse_skills
    return _ok(rid, browse(page=pg, page_size=int(params.get("page_size", 20))))


_SKILLS_ACTIONS = {
    "list": lambda rid, params, query: _ok(rid, {"skills": _tools_mod("sage_cli.banner").get_available_skills()}),
    "search": _skills_search, "install": _skills_install, "browse": _skills_browse,
    "inspect": lambda rid, params, query: _ok(
        rid, {"info": _tools_mod("sage_cli.skills_hub").inspect_skill(query) or {}})}
...
# Gateway mirrors of the dashboard REST surface (sage_cli/web_routers/mcp.py) so a
# desktop plugin can manage MCP servers for ANY profile. Persistence: sage_cli/mcp_config.py.
@_scoped_rpc("mcp.catalog")
def _(rid, params: dict) -> dict:
    """``{servers: [{name, description, installed, enabled, requires: [env keys], transport}]}`` per profile."""
    mcp_catalog = _tools_mod("sage_cli.mcp_catalog")
    out = []
...
def _(rid, params: dict) -> dict:
    ...
    enabled, tools}]}``"""
    servers = _tools_mod("sage_cli.mcp_config")._get_mcp_servers()
...
def _(rid, params: dict) -> dict:
    ...
    import time
    hc = _tools_mod("sage_constants")
    configured = _tools_mod("sage_cli.mcp_config")._get_mcp_servers()
...
def _(rid, params: dict) -> dict:
    ...
    mc = _tools_mod("sage_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    hc, mc = _tools_mod("sage_cli.config"), _tools_mod("sage_cli.mcp_config")
...
def _(rid, params: dict) -> dict:
    ...
    if not _tools_mod("sage_cli.mcp_config")._remove_mcp_server(name):
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        cfg["auth"] = "oauth"
        hermes_home = str(_tools_mod("sage_constants").get_hermes_home().expanduser().resolve(strict=False))
...
def _(rid, params: dict) -> dict:
    ...
    home = str(_tools_mod("sage_constants").get_hermes_home().expanduser().resolve(strict=False))
...
def _plugin_rows() -> list[dict]:
    pc = _tools_mod("sage_cli.plugins_cmd")
    cat = _tools_mod("sage_cli.plugins_cmd_catalog")
...
def _plugins_toggle(rid, params):
    ...
    toggle = _tools_mod("sage_cli.plugins_cmd").dashboard_set_agent_plugin_enabled
...
def _plugins_install(rid, params):
    ...
    result = _tools_mod("sage_cli.plugins_cmd").dashboard_install_plugin(
...
def _plugins_update(rid, params):
    ...
    pc, cat = _tools_mod("sage_cli.plugins_cmd"), _tools_mod("sage_cli.plugins_cmd_catalog")
