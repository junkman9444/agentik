...
Every handler is a thin wrapper around ``sage_cli.kanban_db`` (the same code paths the CLI
...
from sage_cli.kanban_db import KANBAN_ATTACHMENT_MAX_BYTES, _collision_free_path, _safe_attachment_name
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from sage_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    """Run a slow auxiliary-LLM task helper (``sage_cli.<module>.<fn>``) with the board pinned;
    the module is imported lazily so a missing aux client can't break plugin load."""
    def _run():
        return getattr(importlib.import_module(f"sage_cli.{module}"), fn)(task_id, author=(author or None))
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from sage_cli.kanban import _check_dispatcher_presence
...
async def export_board_endpoint(slug: str, body: ExportBoardBody):
    ...
    from sage_cli import kanban_transfer
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    with _errors_to_500("failed to update profile"):
        ...
        if canon == "default":
            from sage_constants import get_hermes_home  # type: ignore
