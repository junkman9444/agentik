...
async def setup(url):
    ...
    runner.session_store = SessionStore(Path(os.environ['AGENTIK_HOME']) / 'sessions', GatewayConfig())
...
def rows():
    with sqlite3.connect(Path(os.environ['AGENTIK_HOME']) / 'state.db') as conn:
