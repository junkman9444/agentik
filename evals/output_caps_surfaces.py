...
def exercise_surfaces(agent, captures, url, home, config):
    ...
    start = len(captures)
    proc = subprocess.Popen([sys.executable, "-m", "sage_cli.main", "chat", "--provider", "fixture-local", "-m", "fixture", "-q", "Return fixture"], stdin=slave, stdout=slave, stderr=slave, cwd=os.getcwd(), env=env)
    os.close(slave)
    output = bytearray()
