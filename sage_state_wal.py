"""SQLite journal-mode and PRAGMA policy for state.db (split from sage_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``sage_state_wal.<name>``.
"""
...
from sage_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable
...
# line would repeat per connection). Tests clear these via ``sage_state_wal.<name>``.
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    and ``sage_cli.kanban_db_connect.connect``. WAL-incompatible filesystems either raise ``OperationalError`` ("locking protocol" / "disk I/O error") or —
...
def _wal_reset_repair_hint() -> str:
    ...
    """
    try:
        from sage_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    try:
        from sage_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
