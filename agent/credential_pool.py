...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.sage/auth.json`` even when HERMES_HOME
...
class CredentialPool(CredentialPoolAdminMixin):
    ...
    def _commit_anthropic_rotation(
        ...
        and other profiles see it). hermes_pkce -> ~/.sage/.anthropic_oauth.json
        ...
        if entry.source == "claude_code":
            ...
            store = "~/.sage/.anthropic_oauth.json"
        else:
            return
        try:
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    ...
    # ~/.sage/.env over os.environ, as `_seed_from_env` does.
    _env_file = load_env()
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.sage/.env over os.environ.
